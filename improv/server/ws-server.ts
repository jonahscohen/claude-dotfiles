...
`);
  ...
  private handleHttpRequest(req: IncomingMessage, res: ServerResponse): void {
    try {
      ...
      if (req.method === 'GET' && bundlePath === '/improv-core.js') {
      ...
      if (req.method === 'GET' && bundlePath.startsWith('/improv-') && bundlePath.endsWith('.js')) {
        ...
        req.on('end', () => {
          try {
            ...
            if (!file.startsWith('/')) {
              ...
              try {
                const improvFiles = execFileSync('find', [
                  join(process.env.HOME || '', 'Documents', 'Github'),
                  '-name', '.improv', '-maxdepth', '2', '-type', 'f'
                ], { encoding: 'utf-8', timeout: 2000 }).trim().split('\n').filter(Boolean);
                for (const imp of improvFiles) {
            ...
            process.stderr.write(`[improv] open-file: resolved "${file}" -> "${fullPath}" cmd="${cmd}"\n`);
            let args: string[];
            if (cmd === 'open') {
              args = ['-R', fullPath];
            } else if (cmd === 'code') {
            ...
            execFile('open', args, (err, stdout, stderr) => {
              if (err) process.stderr.write(`[improv] open-file error: ${err.message}\n`);
              if (stderr) process.stderr.write(`[improv] open-file stderr: ${stderr}\n`);
            });
            res.writeHead(200, { 'Content-Type': 'application/json' });
