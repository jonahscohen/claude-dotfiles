...
export function isImprovElement(node: Node | null): boolean {
  if (!node) return false;
  let e: Node | null = node;
  for (; e;) {
    if (e instanceof HTMLElement && e.hasAttribute("data-improv")) return true;
...
export class PromptMode {
  ...
  activate() {
    ...
    (window as any).__improv_enablePointerBlock();
    ...
    this._lasso = new (window as any).__improv_LassoSelect(this.overlay.getContainer());
  ...
  deactivate() {
    if (!this.active) return;
    this.active = false;
    (window as any).__improv_disablePointerBlock();
    this.overlay.hideHighlight();
  ...
  _onClick(e: MouseEvent) {
    if (isImprovElement(e.target as Node)) return;
    ...
    var t = (window as any).__improv_elementFromPoint(e.clientX, e.clientY);
  ...
  _onHover(e: MouseEvent) {
    if (!this.active) return;
    if (isImprovElement(e.target as Node)) {
      this.overlay.hideHighlight();
      if (this._hLabel) this._hLabel.style.opacity = "0";
      return;
    }
    var t = (window as any).__improv_elementFromPoint(e.clientX, e.clientY) as HTMLElement | null;
    if (!t || t === document.documentElement || t === document.body || t.closest && t.closest("[data-improv]") || this.multiSelect.getAll().some(function (s: SelectedElement) {
      return s.domNode === t;
    })) {
      this.overlay.hideHighlight();
      if (this._hLabel) this._hLabel.style.opacity = "0";
  ...
  buildSelectedElement(e: Element): SelectedElement {
    let t = (window as any).__improv_generateSelector(e),
      n = (window as any).__improv_getComputedStylesFiltered(e),
  ...
  submitPrompt(e: string) {
    ...
    let o = t.map(r => {
      let s = (window as any).__improv_getNearbyText(r.domNode),
        a = (window as any).__improv_getAccessibility(r.domNode),
