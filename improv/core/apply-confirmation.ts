...
export class ApplyConfirmation {
  ...
  constructor(shadowRoot: ShadowRoot) {
    ...
    this.styleEl.textContent = [
      '@keyframes improv-toast-in {',
      '  from { opacity: 0; transform: translateY(-20px); }',
      '  to   { opacity: 1; transform: translateY(0); }',
      '}',
      '@keyframes improv-toast-out {',
      '  from { opacity: 1; transform: translateY(0); }',
      '  to   { opacity: 0; transform: translateY(-10px); }',
      '}',
      '@keyframes improv-pulse-border {',
  ...
  showSending(message: string): void {
    ...
    this.applyStyles(this.container, {
      ...
      animation: 'improv-toast-in 200ms ease-out forwards, improv-pulse-border 1.2s ease-in-out infinite',
    });
  ...
  showSuccess(items: ConfirmationItem[]): void {
    ...
    this.applyStyles(this.container, {
      ...
      animation: 'improv-toast-in 200ms ease-out forwards',
    });
  ...
  private animateDismiss(): void {
    this.container.style.animation = 'improv-toast-out 300ms ease-in forwards';
    const onEnd = (): void => {
