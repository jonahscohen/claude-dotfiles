export class InlinePrompt {
  ...
  constructor(shadowRoot: ShadowRoot | HTMLElement) {
    ...
    this.input.addEventListener("input", () => {
      ...
      if (hasText && !this._btnActive) {
        ...
        this.queueBtn.style.animation = "improv-send-pulse 0.6s ease";
        this.sendNowBtn.style.animation = "improv-send-pulse 0.6s ease 0.1s";
    ...
    this.sendNowBtn.addEventListener("mouseenter", function (this: InlinePrompt) {
      ...
      this.sendNowBtn.querySelector("svg")!.style.animation = "improv-icon-hover-nudge 0.7s cubic-bezier(0.23,1,0.32,1)";
    }.bind(this));
  ...
  setMarkerColor(c: string) {
    ...
    if (this.input) this.input.style.setProperty("--improv-glow-color", c);
    if (this._btnActive) {
      this.queueBtn.style.background = c;
      this.queueBtn.style.borderColor = c;
