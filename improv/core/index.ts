...
function detectImprovUrl(): { port: number; baseUrl: string } {
  const scripts = document.querySelectorAll('script[src*="improv-core"]');
  let port = 9223;
  let isHttps = false;
...
export class ImprovCore {
  ...
  private improvePort: number;
  private improveBaseUrl: string;
  ...
  constructor() {
    const urlInfo = detectImprovUrl();
    this.improvePort = urlInfo.port;
    this.improveBaseUrl = urlInfo.baseUrl;
    this.transport = new Transport(this.improvePort);
  ...
  async init(): Promise<void> {
    ...
    this.transport.on('improv_working', () => {
    ...
    this.transport.on('improv_response', (data: unknown) => {
      ...
      try {
        fetch(`${this.improveBaseUrl}/responses`,{method:'POST',body:JSON.stringify(this._changeHistory)}).catch(()=>{});
      } catch {}
    ...
    fetch(`${this.improveBaseUrl}/responses`).then(r => r.json()).then((data: any[]) => {
    ...
    setTimeout(() => {
    fetch(`${this.improveBaseUrl}/queue`).then(r => r.json()).then((data: any[]) => {
  ...
  activate(): void {
    ...
    if (!this._queuePill) {
      ...
      this._queuePill.dataset.improv = '';
    ...
    this._changesPanel.setOnReply((promptId: string, text: string) => {
      ...
      this.transport.request('push_prompt', promptData).catch((e: unknown) => {
        console.warn('[Improv] Reply failed:', e);
        this._claudeToRetry();
      });
    });
    ...
    this._changesPanel.setOnHide(() => {
      ...
      if (this._claudePill && this._claudeState === 'review-active') {
        ...
        if (this._claudeSpark) this._setSpark(this._claudeSpark, ImprovCore._staticSvg, '#D97757');
      }
    });
    ...
    this._changesPanel.setOnRevert((promptId: string, changes: any[]) => {
      ...
      this._showClaudeBar('Sending to Claude', 'writing', true);
      this.transport.request('push_prompt', promptData).catch((e: unknown) => {
        console.warn('[Improv] Revert prompt failed:', e);
        this._claudeToRetry();
      });
    });
  ...
  private _loadClaudeState(): void {
    fetch(`${this.improveBaseUrl}/watch-status`)
      .then(r => r.json())
      .then((watchData: { active: boolean }) => {
        if (!watchData.active) return;
        return fetch(`${this.improveBaseUrl}/claude-state`).then(r => r.json());
      })
      .then((data: any) => {
  ...
  private _showDisconnectedBar(): void {
    ...
    pill.style.cssText =
      ...
      'animation:improv-claudebar-glow 2s ease-in-out infinite;' +
    ...
    icon.insertAdjacentHTML('beforeend', ImprovCore._staticSvg);
    ...
    tip.style.cssText =
      ...
      'font-size:11px;font-family:ImprovSans,system-ui,sans-serif;color:rgba(255,255,255,0.7);' +
      'white-space:nowrap;pointer-events:none;opacity:0;transition:opacity 150ms ease,transform 150ms ease;' +
      'box-shadow:0 4px 16px rgba(0,0,0,0.4)';
    tip.textContent = 'Claude disconnected. Say "watch improv" to reconnect.';
  ...
  private _loadSprites(): void {
    ...
    ['writing', 'shimmer', 'waiting', 'thinking'].forEach(name => {
      fetch(`${this.improveBaseUrl}/spark-${name}.svg`).then(r => r.text()).then(t => { this._spriteSvgs[name] = t; }).catch(() => {});
    });
  }
  ...
  private _animateSpark(container: HTMLDivElement, spriteName: string): Animation | null {
    ...
    const cfg = ImprovCore._spriteConfigs[spriteName];
    ...
    if (spriteName === 'waiting') {
      frames = ImprovCore._waitingFrameIndices.map(i => ({ transform: `translateY(-${i * (100 / cfg.frameCount)}%)` }));
  ...
  _showClaudeBar(text: string, sprite: string, hasDots: boolean): void {
    ...
    label.style.cssText =
      ...
      'transition:width 0.35s cubic-bezier(0.4,0,0.2,1),opacity 0.25s ease;font-family:ImprovSans,system-ui,sans-serif';
    label.textContent = text;
    if (hasDots) {
      const dots = document.createElement('span');
      dots.style.cssText = 'display:inline-block;width:1.2em;text-align:left;vertical-align:baseline';
      const style = document.createElement('style');
      style.textContent = '@keyframes improv-ellipsis{0%{content:""}25%{content:"."}50%{content:".."}75%{content:"..."}}';
  ...
  _claudeToRetry(): void {
    ...
    this._claudePill.style.animation = 'improv-claudebar-glow 3s ease-in-out infinite';
    if (!document.getElementById('improv-claudebar-glow-style')) {
      const s = document.createElement('style');
      s.id = 'improv-claudebar-glow-style';
      s.textContent = '@keyframes improv-claudebar-glow{0%,100%{border-color:rgba(255,255,255,0.1);box-shadow:0 2px 12px rgba(0,0,0,0.4)}50%{border-color:rgba(217,119,87,0.25);box-shadow:0 2px 12px rgba(0,0,0,0.4),0 0 12px rgba(217,119,87,0.06)}}';
  ...
  private _retryClaudeBarClick(): void {
    ...
    this.transport.request('push_prompt', this._lastPromptData)
      .catch((e: unknown) => { console.warn('[Improv] Retry failed:', e); });
  ...
  private _claudeBarClick(): void {
    ...
    if (this._claudeState === 'review') {
      ...
      if (this._claudeAnim) { this._claudeAnim.cancel(); this._claudeAnim = null; }
      this._setSpark(this._claudeSpark, ImprovCore._staticSvg, '#fff');
      ...
      if (isOpen) {
        this._changesPanel?.hide();
        if (icon) { icon.style.background = 'rgba(255,255,255,0.08)'; icon.style.color = '#D97757'; }
        this._setSpark(this._claudeSpark, ImprovCore._staticSvg, '#D97757');
      } else {
        const actionable = this._changeHistory.filter(e => !e.reviewed);
        if (actionable.length > 0) {
          this._changesPanel?.toggle(this._changeHistory as any);
  ...
  _buildQueueRow(idx: number, item: QueueItem, isLast: boolean): HTMLDivElement {
    ...
    numSpan.style.cssText = 'font-size:10px;font-weight:700;color:#1a1a1a;font-variant-numeric:tabular-nums;font-family:ImprovSans,system-ui,sans-serif';
    ...
    target.style.cssText = 'font-size:10px;color:rgba(255,255,255,0.35);margin-top:3px;font-family:ImprovSans,system-ui,sans-serif';
