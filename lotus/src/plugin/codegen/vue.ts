export function exportVue(node: SceneNode): string {
  const componentName = sanitizeName(node.name);
  const template = nodeToTemplate(node, 1);
...
function nodeToTemplate(node: SceneNode, indent: number): string {
  ...
  const children = (node as FrameNode).children
    ...
    .map(c => nodeToTemplate(c as SceneNode, indent + 1))
    .join('\n');
...
function buildClasses(node: SceneNode): string {
  ...
  if ('cornerRadius' in node) {
    ...
    if (typeof cr === 'number' && cr > 0) classes.push(`rounded-[${cr}px]`);
  }
  return classes.join(' ');
}
