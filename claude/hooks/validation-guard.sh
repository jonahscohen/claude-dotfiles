...
# Calling methods on a known application namespace (justify-specific for now;
# add others as the codebase grows). The pattern: <namespace>.method(...).
if [ -z "$REASON" ] && echo "$JS_CODE" | grep -qE '(window\.)?__justify\.[a-zA-Z_][a-zA-Z0-9_]*\s*\('; then
  REASON="Invoking a method on the application namespace (__justify) skips the user-facing path. The very state transitions you want to verify are now triggered by you, not by the UI."
