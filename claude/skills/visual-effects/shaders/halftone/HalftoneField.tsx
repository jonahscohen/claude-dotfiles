// Halftone field reference implementation.
// CMYK halftone dot-screen effect over a fluid-simulated color field.
// Tech: Three.js (WebGLRenderer, ShaderMaterial for halftone pass)
//
// SCENE_PRESETS is not included here - presets are color sets.
// Each preset provides 4 base colors for the background gradient.
...
function resolvePreset(params: typeof DEFAULT_PARAMS): ScenePreset {
  if (params.scene === -1 || params.customBaseColor1) {
    const fallback = FALLBACK_PRESET;
    return {
      name: "Custom",
      baseColor1: params.customBaseColor1 ?? fallback.baseColor1,
      baseColor2: params.customBaseColor2 ?? fallback.baseColor2,
      baseColor3: params.customBaseColor3 ?? fallback.baseColor3,
      baseColor4: params.customBaseColor4 ?? fallback.baseColor4,
      // Env colors unused by halftone renderer but required by ScenePreset type
      envColor1: fallback.envColor1,
      envColor2: fallback.envColor2,
      envColor3: fallback.envColor3,
    };
  }
...
void main() {
  ...
  // Per-channel pow() compresses ratios toward grey - this preserves them
  ...
  // Invert if requested (large dots on dark, small on bright)
  ...
  // Subtle dark floor: just enough to hint at color between dots
  // The dots carry the color - the gaps stay dark to preserve the halftone punch
