...
3. **Interaction patterns** - What states does this component need? Hover, focus, active, disabled, loading, error, empty. What keyboard shortcuts are expected? What happens on Escape, Enter, Tab, arrow keys? The gallery's cross-system view reveals which interactions are considered table stakes vs. nice-to-have.
   ...
   **Hover-activated menus - the mouse safe area.** For any nested/hover-triggered menu (dropdown menu, navigation flyouts, nested context menus, hover popovers), there is one interaction detail the gallery's per-system pages rarely call out but every good implementation needs: the mouse safe area, also called the "safe triangle." When a submenu opens to the side of its trigger, a user sweeping the cursor diagonally toward it briefly leaves the trigger's bounds, and a naive `onMouseLeave` closes the submenu before they arrive. The fix is an invisible, `clipPath`-clipped area spanning the gap between the cursor and the submenu, keeping the parent "hovered" during the diagonal travel. Treat it as table stakes for hover menus, alongside focus trapping for modals. Two guardrails: (a) it is pointer-only, so it must never be the *only* path into a submenu - full keyboard navigation (arrow keys to move, Enter/Right to open, Esc/Left to close) and touch/click access are still required; (b) prefer a maintained library that ships this behavior (Radix UI, Base UI, Floating UI) over a bespoke global mouse-position hook. If you must hand-roll it, the canonical reference is eldh's `MouseSafeArea` (an absolutely-positioned div whose left/right/width/clipPath are derived from the cursor position and the submenu's bounding rect): https://gist.github.com/eldh/51e3825b7aa55694f2a5ffa5f7de8a6a
