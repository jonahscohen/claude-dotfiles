...
async function run() {
  console.log('Connecting to justify server...');
  ...
  const responsePromise = new Promise((resolve) => {
    ws.on('message', (data) => {
      const msg = JSON.parse(data.toString());
      if (msg.method === 'justify_response') {
        resolve(msg.params);
      }
    });
  });
  ...
  // The server's justify_respond tool broadcasts to all clients
  ...
  // The real flow would be: Claude calls justify_respond MCP tool -> server broadcasts
  ...
  console.log('  - Response listener ready for justify_response events');
  console.log('\nNote: Full loop test requires Claude Agents session with justify MCP tools');
