...
if [ -f "vite.config.ts" ] || [ -f "vite.config.js" ] || [ -f "vite.config.mjs" ]; then
  if [ -f "index.html" ]; then
    if ! grep -q "justify-core" "index.html"; then
      sed -i.bak 's|</head>|  <script src="'"$JUSTIFY_URL"'"></script>\n  </head>|' index.html
      rm -f index.html.bak
  ...
  if [ -n "$LAYOUT" ]; then
    if ! grep -q "justify-core" "$LAYOUT"; then
      if ! grep -q "next/script" "$LAYOUT"; then
        sed -i.bak '1s|^|import Script from "next/script";\n|' "$LAYOUT"
        rm -f "${LAYOUT}.bak"
      fi
      sed -i.bak 's|</body>|<Script src="'"$JUSTIFY_URL"'" strategy="afterInteractive" />\n      </body>|' "$LAYOUT"
      rm -f "${LAYOUT}.bak"
      echo "Added Justify Script to $LAYOUT"
    fi
  else
    echo "WARNING: Could not find Next.js layout file. Add manually:"
    echo "  <Script src=\"$JUSTIFY_URL\" strategy=\"afterInteractive\" />"
  fi
  ...
  if [ -f "${THEME_NAME}.libraries.yml" ]; then
    if ! grep -q "justify-dev" "${THEME_NAME}.libraries.yml"; then
...
DEOF
      echo "Added justify-dev library to ${THEME_NAME}.libraries.yml"
    fi
  fi
  ...
  if [ -n "$TEMPLATE" ]; then
    if ! grep -q "justify-dev" "$TEMPLATE"; then
      sed -i.bak '1s|^|{{ attach_library('"'"${THEME_NAME}'/justify-dev'"'"') }}\n|' "$TEMPLATE"
      rm -f "${TEMPLATE}.bak"
      echo "Added attach_library to $TEMPLATE"
    fi
  else
    echo "Add to your page template: {{ attach_library('${THEME_NAME}/justify-dev') }}"
  fi
...
define('WP_DEBUG', true);" wp-config.php 2>/dev/null || echo "define('WP_DEBUG', true);" >> wp-config.php
  ...
  if [ -n "$FUNCS" ]; then
    if ! grep -q "justify-dev" "$FUNCS"; then
      cat >> "$FUNCS" << WEOF
...
WEOF
  ...
  if [ -n "$HTML" ]; then
    if ! grep -q "justify-core" "$HTML"; then
      sed -i.bak 's|</head>|  <script src="'"$JUSTIFY_URL"'"></script>\n  </head>|' "$HTML"
      rm -f "${HTML}.bak"
    ...
    echo "  <script src=\"$JUSTIFY_URL\"></script>"
  fi
fi
