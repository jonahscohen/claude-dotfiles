...
export class PromptMode {
  ...
  activate() {
    ...
    (window as any).__justify_enablePointerBlock();
    ...
    this._lasso = new (window as any).__justify_LassoSelect(this.overlay.getContainer());
    ...
    this._sendAllBtn.addEventListener("mouseenter", function (this: PromptMode) {
      ...
      _saSvg.style.animation = "justify-icon-hover-nudge 0.7s cubic-bezier(0.23,1,0.32,1)";
    ...
    this._clearAllBtn.addEventListener("mouseenter", function (this: PromptMode) {
      ...
      _caSvg.style.animation = "justify-icon-hover-shake 0.4s cubic-bezier(0.23,1,0.32,1)";
  ...
  deactivate() {
    if (!this.active) return;
    this.active = false;
    (window as any).__justify_disablePointerBlock();
    this.overlay.hideHighlight();
  ...
  _onClick(e: MouseEvent) {
    ...
    var t = (window as any).__justify_elementFromPoint(e.clientX, e.clientY);
  ...
  _onHover(e: MouseEvent) {
    ...
    var t = (window as any).__justify_elementFromPoint(e.clientX, e.clientY) as HTMLElement | null;
  ...
  buildSelectedElement(e: Element): SelectedElement {
    let t = (window as any).__justify_generateSelector(e),
      n = (window as any).__justify_getComputedStylesFiltered(e),
  ...
  submitPrompt(e: string) {
    ...
    let o = t.map(r => {
      let s = (window as any).__justify_getNearbyText(r.domNode),
        a = (window as any).__justify_getAccessibility(r.domNode),
    ...
    if (this._queueCount) {
      ...
      if (this._actionPill) {
        if (c > 0) {
          ...
          this._actionPill.style.animation = "justify-send-pulse 0.5s cubic-bezier(0.23,1,0.32,1)";
        } else {
