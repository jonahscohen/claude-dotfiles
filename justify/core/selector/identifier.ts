 ...
 *
 * Ported near-verbatim from Retune (packages/overlay/src/selector/identifier.ts)
 * as framework-free vanilla TS. Depends only on @medv/finder (MIT) and parsel-js
 * (MIT), both already Justify dependencies. The React-fiber helpers
 * (getReactComponentHierarchy, getDirectReactProps, setReactProp, ...) are
 * vanilla TS that introspect the `__reactFiber$*` DOM keys only when present;
 * on a non-React page they gracefully return [] / null (i.e. optional no-ops).
