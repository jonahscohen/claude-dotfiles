 ...
 *
 * Ported verbatim-in-behavior from Retune (overlay/src/inspector/styles.ts).
 * This module is framework-free: it relies only on DOM/CSSOM APIs and the
 * local camelToKebab helper. No React-fiber dependencies exist here, so no
 * function needed to be stubbed to a no-op for the vanilla/WordPress target.
 * (Comments de-emdashed to satisfy the repo content guard; behavior is identical.)
 */
...
export function getScopedStyles(
  ...
  function walkScopedRules(rules: CSSRuleList) {
    for (let i = 0; i < rules.length; i++) {
      ...
      // Include rules where all classes are within scope (rule subset of scope)
      // OR rules that are a superset of scope (scope subset of rule) - these have
