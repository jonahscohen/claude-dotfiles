...
function isJustifyElement(el: HTMLElement | null): boolean {
  if (!el) return false;
  let node: Node | null = el;
  while (node) {
    if (node instanceof HTMLElement && node.hasAttribute('data-justify')) return true;
...
export class AnnotateMode {
  ...
  private _onClick(e: MouseEvent): void {
    if (isJustifyElement(e.target as HTMLElement)) return;
    e.preventDefault();
    e.stopPropagation();
