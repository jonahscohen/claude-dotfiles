...
export class ComponentPalette {
  ...
  private build(): void {
    ...
    style.textContent = [
      '@keyframes justify-palette-in {',
      '  from { opacity: 0; filter: blur(5px); }',
      '  to { opacity: 1; filter: blur(0); }',
      '}',
      '.justify-palette-scroll::-webkit-scrollbar { width: 3px; }',
      '.justify-palette-scroll::-webkit-scrollbar-track { background: transparent; }',
      '.justify-palette-scroll::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.15); border-radius: 2px; }',
    ].join('\n');
    ...
    Object.assign(this.panel.style, {
      ...
      userSelect:    'none',
      animation:     'justify-palette-in 0.2s ease',
    });
    ...
    scrollArea.className = 'justify-palette-scroll';
    ...
    // Dot-grid background overlay (appended to document.body so it's between page and justify overlay)
    this.dotGridEl = document.createElement('div');
    this.dotGridEl.dataset['justifyGrid'] = '';
    ...
    this.opacityOverlayEl.dataset['justifyOpacity'] = '';
  ...
  private buildItem(name: string, category: string): HTMLDivElement {
    ...
    item.addEventListener('dragstart', (e: DragEvent) => {
      ...
      e.dataTransfer.setData('justify/type', name);
      e.dataTransfer.setData('justify/category', category);
  ...
  private buildFooter(): HTMLDivElement {
    ...
    const sliderId = 'justify-opacity-slider-' + Date.now();
  ...
  private handleDragOver = (e: DragEvent): void => {
    if (!e.dataTransfer?.types.includes('justify/type')) return;
    e.preventDefault();
  ...
  private handleDrop = (e: DragEvent): void => {
    ...
    const type = e.dataTransfer.getData('justify/type');
    const category = e.dataTransfer.getData('justify/category');
