...
const cssVars = {
  surface: '--justify-surface',
  surfaceHover: '--justify-surface-hover',
  text: '--justify-text',
  textSecondary: '--justify-text-secondary',
  textTertiary: '--justify-text-tertiary',
  border: '--justify-border',
  inputBg: '--justify-input-bg',
  blueBg: '--justify-blue-bg',
  blueText: '--justify-blue-text',
  blue500: '--justify-blue-500',
  surfaceActive: '--justify-surface-active',
  black: '--justify-black',
  white: '--justify-white',
};
...
const FONT_FAMILY = "'JustifySans', system-ui, sans-serif";
...
.justify-pp-panel::-webkit-scrollbar { display: none; }`;
    this.shadow.appendChild(style);
    this.container.classList.add('justify-pp-panel');
  }
  ...
  private buildTree(el: HTMLElement, depth: number): TreeNode {
    ...
    for (let i = 0; i < el.children.length; i++) {
      ...
      if (!child || child.nodeType !== 1 || child.tagName === 'JUSTIFY-PANEL' || child.classList.contains('justify-pp-panel')) continue;
