 ...
 *
 * 1:1 React->Preact port of Retune's packages/overlay/src/overlay/ElementTree.tsx
 * (spec docs/retune-port/12-ELEMENTS-TAB.md). Logic + inline SVG icon path data are
 * verbatim. Class names (.retune-tree*) are preserved so the ported CSS matches.
 *
 * Port deltas from the Retune source (only three touch logic):
 *  - Import path: getDirectReactComponent from ../../selector/identifier.js (our tree).
 *  - R2 (Preact events): Retune passed `e.nativeEvent` to onDragStart; under Preact the
 *    handler receives the native event directly, so we pass `(e as any).nativeEvent ?? e`.
 *    This is the only React-only synthetic-event assumption in the file (audited).
 *  - R3 (overlay exclusion): Retune's `isRetuneElement` is re-pointed to Justify's overlay
 *    markers (`isJustifyOverlayElement`) so our toolbar/panel/picker chrome never appears
 *    as tree rows.
 *  - R8 (brand hygiene): internal `data-retune-tree-key` attribute renamed to
 *    `data-justify-tree-key` (writer + drag hit-test reader kept in sync).
 *
 * `react` resolves to `preact/compat` via the esbuild alias (build.js), so the verbatim
 * Retune import line is kept intact.
 *
 * Phase A: onTreeReorder / onTreeReparent are intentionally left unwired by the caller.
 * handleDragStart early-returns when both are undefined, so pointerdown falls through to
 * click-select and the drag machinery below is dormant (no dead UI). Drag is wired in Phase B.
 */
...
/** Check if element is part of Justify's own overlay chrome.
 *  Re-pointed from Retune's isRetuneElement (R3): Justify marks every overlay node with a
 *  `data-justify*` attribute - the panel host (`data-justify`), picker highlight/label/
 *  selection/parent-indicator/drag-ghost/reparent-line/cursor, freeze + queue markers - and
 *  mounts the picker's isolated shadow host as `[data-justify-host]`. Excluding any element
 *  carrying a `data-justify` / `data-justify-*` attribute (or living under the picker host)
 *  keeps all Justify chrome out of the tree. */
function isJustifyOverlayElement(el: Element): boolean {
  for (const name of el.getAttributeNames()) {
    if (name === "data-justify" || name.startsWith("data-justify-")) return true;
  }
  if (el.closest("[data-justify-host]")) return true;
  return false;
}

/** Get visible children of an element, filtering out Justify overlay and skip tags.
...
export function getVisibleChildren(el: Element, visualOrderMap?: Map<Element, Element[]>, reparentEntries?: ReparentEntry[]): Element[] {
  ...
  for (const child of source) {
    ...
    if (isJustifyOverlayElement(child)) continue;
  ...
  return (
    <>
      <div
        ...
        data-justify-tree-key={getStableKey(element)}
        ...
        onPointerDown={(e) => {
          ...
          // R2: under Preact the handler receives the native event directly;
          // Retune's `e.nativeEvent` is undefined here. Fall back to the event itself.
...
export function ElementTree({ selectedElement, onSelect, onHover, visualOrderMap, reparentEntries, onTreeReorder, onTreeReparent }: ElementTreeProps) {
  ...
  const findTargetElement = useCallback((e: PointerEvent): { element: Element; treeNode: HTMLDivElement } | null => {
    ...
    const treeNode = hit?.closest?.("[data-justify-tree-key]") as HTMLDivElement | null;
    if (!treeNode) return null;
    const key = parseInt(treeNode.getAttribute("data-justify-tree-key") || "", 10);
  ...
  const handleDragStart = useCallback((e: PointerEvent, element: Element) => {
    // Drag disabled (Phase A: onTreeReorder/onTreeReparent unset). In Retune the
    // click-select lives in the drag pointer-up; with the drag path off we'd lose
    // selection entirely, so provide click-select here - select on a pointer-up that
    // did not move (a click, not a drag). Phase B re-enables the full drag path
    // (which owns click-select) and this branch is skipped.
    if (!onTreeReorder && !onTreeReparent) {
      const sx = e.clientX, sy = e.clientY;
      const onUp = (up: PointerEvent) => {
        document.removeEventListener("pointerup", onUp, true);
        if (Math.abs(up.clientX - sx) < 4 && Math.abs(up.clientY - sy) < 4) onSelect(element);
      };
  ...
  }, [onTreeReorder, onTreeReparent, visualOrderMap, handlePointerMove, handlePointerUp, onSelect]);
