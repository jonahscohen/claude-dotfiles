 ...
 * (via Retune overlay/src/ui/color-utils.ts), verbatim-in-behavior.
 * Pure logic - no DOM. (Comments de-emdashed for the content guard.)
 *
 * EXACT numeric behavior preserved (plan open-question n):
 *  - parseCssColor returns opacity as an INTEGER percent (0-100).
 *  - hexToRgba serializes alpha to 2 decimal places, and returns a PLAIN hex
 *    (no alpha) when opacity >= 100.
 */
