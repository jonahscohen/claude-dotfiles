 ...
 * Ported near-verbatim from Retune overlay/src/ui/font-input.tsx.
 *
 * Shows project fonts (scanned from stylesheets), system fonts (Local Font
 * Access API), and generic fallbacks; each rendered in its own typeface.
 *
 * DEVIATIONS from source (both minor, noted):
 *  - Portal target: Retune queries `[data-retune-container]` inside the shadow
 *    root. Justify's overlay container is `.justify-container`; we try
 *    `[data-retune-container]` then `.justify-container` then the shadow root,
 *    and fall back to document.body when NOT in a shadow root (so the control is
 *    usable in a plain page / sandbox). Source returned null outside a shadow root.
 *  - Dropped the unused `Tooltip` import that Retune imports but never renders.
 *  - The empty-trigger placeholder is the en-dash glyph (U+2013), written as
 *    String.fromCharCode(0x2013) to match the other controls (and the repo's
 *    no-literal-dash rule); identical rendered output to Retune.
 */
...
import { claimDialog, releaseDialog } from "./dialog-singleton.js";
import { ChangeIndicator } from "./change-indicator.js";
...
export function FontInput({ prop, value, onChange, isChanged, onReset }: FontInputProps) {
  ...
  const [, setLocalValue] = useState(primaryFont);
  ...
  // Find portal target for the floating dialog. Retune queries
  // [data-retune-container] inside the shadow root; Justify's container is
  // .justify-container. Fall back to document.body outside a shadow root.
  const portalTarget = (() => {
    const root = containerRef.current?.getRootNode();
    if (root instanceof ShadowRoot) {
      return (root.querySelector("[data-retune-container]")
        || root.querySelector(".justify-container")
  ...
  return (
    <div className="retune-font-input" ref={containerRef}>
      ...
      <button
        ...
        onClick={openPicker}
      >
        <span className="retune-font-input-value" style={{ fontFamily: primaryFont || undefined }}>{primaryFont || PLACEHOLDER_DASH}</span>
        <ChevronDown />
      </button>
