 ...
 *
 * Ported 1:1 from Retune ui/sections/PositionSection.tsx per spec
 * 03-sections-position-layout-spacing-size.md (section 1). React -> Preact:
 * hooks from preact/hooks; token/variable branches DEFERRED (none in Position).
 * Pin authored-state detection reads the live DOM node when present, else falls
 * back to the default {top,left} pins so the section still renders with a mock.
 */
...
export interface PositionSectionProps extends BaseSectionProps, LayoutContextProps {
  /** Notify parent when authored pin lines change (drives on-page pin overlay). */
  onPinLinesChange?: (authored: { top: boolean; right: boolean; bottom: boolean; left: boolean }) => void;
}

const DEFAULT_PINS: PinState = { top: true, right: false, bottom: false, left: true };
...
export function PositionSection({
  ...
  const [pins, setPins] = useState<PinState>(() => {
    ...
    if (pos !== "absolute" && pos !== "fixed") return { ...DEFAULT_PINS };
    if (!el) return { ...DEFAULT_PINS };
