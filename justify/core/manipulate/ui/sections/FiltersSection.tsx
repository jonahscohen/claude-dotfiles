/**
 * FiltersSection - filter list (blur, brightness, contrast, etc.) with
 * add/remove and backdrop-vs-layer grouping.
 *
 * Ported from Retune ui/sections/FiltersSection.tsx per spec 05 (section 2).
 * React -> Preact. Uses only `s` + `onPropertyChange` (no token/variable layer).
 * Imports filter-utils from core/manipulate/ui/filter-utils (ported task #19).
 * The add-filter menu HIDES already-used filter types per target, so each type
 * appears at most once per layer/backdrop.
 */
...
export function FiltersSection({ s, onPropertyChange }: FiltersSectionProps) {
