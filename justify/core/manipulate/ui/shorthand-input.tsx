/**
 * ShorthandInput - handles comma/space-separated values for multi-property
 * fields like padding (H/V) and border-radius. Displays a merged value when all
 * props match, or comma-separated individual values when they differ.
 *
 * Ported from Retune overlay/src/ui/shorthand-input.tsx.
 *
 * DEVIATION (Phase 3b scope): the variable/token integration (variableMatch,
 * VariableAction) is deferred to the later token phase. The scrub-all-equally
 * math, shorthand commit parsing (single -> all props; multiple -> cycle), the
 * arrow-key stepper, and exact geometry are preserved verbatim. The CALLER
 * decides prop order; for corner-radius that is TL,TR,BR,BL per plan (l).
...
export function ShorthandInput({ label, props, values, onChange, placeholder, min, max, isChanged, onReset }: ShorthandInputProps) {
  ...
  return (
    <div ref={wrapperRef} className="retune-prop">
