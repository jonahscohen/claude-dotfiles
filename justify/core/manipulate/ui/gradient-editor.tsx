 ...
 * Stop bar + angle input + reverse/rotate + stops list (position + ColorInput +
 * remove). All inline, no dialogs. Reuses ColorInput per stop.
 *
 * Ported from Retune overlay/src/ui/gradient-editor.tsx.
 *
 * DEVIATION (Phase 5 scope): per-stop variable/token associations (stopVariables
 * map + the ColorInput onVariableApply/Select/Unlink wiring) are DEFERRED to the
 * later token phase. Stop bar drag, angle input (+/-/normalize, radial hides
 * angle), reverse/rotate, add/remove stop (min 2), per-stop change tracking, and
 * the exact gradientToCss emission (via the parent's onChange) are verbatim.
 */
...
// Degree sign (U+00B0) and en-dash placeholder (U+2013), matching Retune verbatim.
const DEG = "°";
...
export function GradientEditor({ gradient, onChange, originalGradient, isNewGradient }: GradientEditorProps) {
  ...
  const [angleInput, setAngleInput] = useState(`${gradient.angle}${DEG}`);
  ...
  if (gradient.angle !== prevAngle) {
    ...
    if (!isEditingAngle) setAngleInput(`${gradient.angle}${DEG}`);
  }
  ...
  const handleAngleInputChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setAngleInput(raw);
      const cleaned = raw.replace(new RegExp(DEG, "g"), "").trim();
    ...
    <div className="retune-gradient-editor">
      ...
      <div className="retune-gradient-controls">
        <input
          ...
          type="text"
          value={showAngle ? (isEditingAngle ? angleInput : `${gradient.angle}${DEG}`) : DASH}
          readOnly={!showAngle}
          disabled={!showAngle}
          onFocus={showAngle ? (e) => {
            setIsEditingAngle(true);
        ...
        <div className="retune-gradient-actions">
          ...
          <Tooltip content={`Rotate gradient 45${DEG}`}>
            <button
              type="button"
              className="retune-gradient-action-btn"
              disabled={!showAngle}
