 ...
 *
 * Ported faithfully (behavior-identical) from Retune
 * (overlay/src/engine/change-tracker.ts). Self-contained: depends only on the
 * engine change-model types, no Preact panel and no token/candidate layer.
 * Internal property keys stay camelCase; kebab conversion happens only at the
 * output boundary. (Comments de-emdashed for the repo content guard.)
 *
 * INTEGRATION SEAM (later phase, do not wire here): getFlatChanges() emits rows
 * shaped like Justify's ChangeBuffer PendingChange so a future step can feed or
 * supersede ChangeBuffer/Transport with this richer model.
 */

import type { ElementChange, PropertyChange, TrackedVariableRef, FlatChange } from "./types.js";

export type { TrackedVariableRef };
...
export class ChangeTracker {
  ...
  /** Flatten pending changes into ChangeBuffer-shaped rows for the Transport seam.
   *  oldValue=from, newValue=to. CSS property changes only (camelCase keys);
   *  attribute/prop changes are intentionally excluded from this flat view since
   *  ChangeBuffer is CSS-property-keyed. Later integration decides how to merge. */
  getFlatChanges(): FlatChange[] {
    const rows: FlatChange[] = [];
    for (const el of this.getPendingChanges()) {
