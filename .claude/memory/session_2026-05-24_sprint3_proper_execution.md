...
- T2 quality fix: added a clarifying comment to the IDIOMS.unknown record explaining why it intentionally duplicates vanilla content (Record completeness requires the key; accessor `??` is a separate defensive layer for `as any` paths). Also added a JSDoc comment to getMotionIdiom documenting the two-layer guard. No content change; only comments. tsc --noEmit passes, test PASS.
- T3 in progress: failing test created at sidecoach/src/__tests__/sprint3-motion-stack-integration.test.ts. Asserts FlowHMotionIntegrationHandler emits a "Stack-specific implementation (framework=<X>)" block in guidance + a 'template' artifact named "Motion code template: <X>" for drupal, wordpress, and unknown/vanilla fallback contexts.
- T3 red phase verified: test prints "FAIL drupal guidance includes Stack-specific header: got false" - expected failure before wiring getMotionIdiom into the handler.
- T3 step 3a: imported getMotionIdiom from './motion-stack-idioms' at top of flow-handler-motion-integration.ts alongside the other module imports.
- T3 step 3b: appended Phase 4 stack-specific block to guidance array AFTER the closing literal (so existing Sprint 2 T12 DESIGN.md citation lines + motion-domain rules are preserved). Block reads framework from context.metadata.techStack.framework, falls back to 'unknown' which getMotionIdiom resolves to vanilla. Emits header, loadingPattern, cleanupPattern, scopeBoundary, indented snippet lines, plus per-note bullets.
- T3 suppress-fix-gate: touched ~/.claude/.suppress-fix-gate because step 3a (import) + step 3b (guidance append) + step 3c (artifact prepend) are one coherent change required by T3 plan - not three independent fix attempts.
