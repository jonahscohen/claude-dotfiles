...
const python_ast_check_1 = require("../python-ast-check");
const python_sandbox_1 = require("../python-sandbox");
const schemas_1 = require("../schemas");
exports.definition = {
    name: 'sidecoach_python_repl_execute',
    description: 'Execute a one-shot Python snippet inside a locked-down container (docker or podman required on PATH). ' +
    ...
    inputSchema: schemas_1.pythonReplExecuteShape,
...
const handler = async (input, deps) => {
    ...
    const probe = await (0, python_sandbox_1.probeRuntime)();
    if (!probe.ok || !probe.runtime) {
        throw new errors_1.SidecoachToolError('DOWNSTREAM_UNAVAILABLE', `Python REPL requires a container runtime: ${probe.reason ?? 'no docker/podman on PATH'}. ` +
            'Install Docker (https://docs.docker.com/get-docker/) or Podman.', { resource: 'docker' });
    }
    // Layer 1: static screen before the container is spawned.
    const screen = (0, python_ast_check_1.checkPythonCode)(input.code);
    if (!screen.ok) {
        throw new errors_1.SidecoachToolError('INVALID_INPUT', `python code rejected by static screen: ${screen.violation}`, { errorMessage: screen.violation });
    }
    const image = (0, python_sandbox_1.resolveImage)();
    const timeoutMs = input.timeoutMs ?? python_sandbox_1.CONTAINER_TIMEOUT_MS;
    CALL_SEQ += 1;
    const containerName = `sidecoach-pyrepl-${process.pid}-${Date.now()}-${CALL_SEQ}`;
    const outcome = await (0, python_sandbox_1.runInContainer)({
    ...
    if (outcome.spawnError && outcome.spawnError.code === 'ENOENT') {
        ...
        throw new errors_1.SidecoachToolError('DOWNSTREAM_UNAVAILABLE', `${probe.runtime} runtime vanished mid-call`, { resource: 'docker' });
    }
    if (outcome.daemonDown) {
        throw new errors_1.SidecoachToolError('DOWNSTREAM_UNAVAILABLE', `${probe.runtime} daemon is not reachable. Is it running?`, { resource: 'docker' });
    }
    if (outcome.timedOut) {
        throw new errors_1.SidecoachToolError('TIMEOUT', `python execution exceeded ${timeoutMs}ms; container killed`, { timeoutMs });
    }
    const data = {
        image,
        runtime: probe.runtime,
...
//# sourceMappingURL=python-repl-execute.js.map