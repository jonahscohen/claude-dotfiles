 ...
 */
export function parseAstGrepStream(raw: string, opts: ParseOptions): ParseResult {
  ...
  for (const line of lines) {
    ...
    const range = obj.range as
      | { start?: { line?: unknown; column?: unknown }; end?: { line?: unknown; column?: unknown } }
      | undefined;
    if (
    ...
    matches.push({
      ...
      startColumn:
        typeof range.start.column === 'number' ? Number(range.start.column) : 0,
      endColumn:
        typeof range.end.column === 'number' ? Number(range.end.column) : 0,
