...
exports.TIER2_VISUAL_COPY_RULES = exports.COPYWRITING_RULES = exports.CHAR_SUBSTITUTION_RULES = exports.TIER2_MOTION_RULES = exports.CHART_SELECTION_RULES = exports.CHART_SELECTION_MATRIX = exports.DARK_MODE_RULES = void 0;
...
function visibleText(ctx) {
    ...
    if (ctx.htmlElement) {
        try {
            raw += ctx.htmlElement.textContent || '';
        }
        catch { /* no DOM */ }
    }
    raw = raw
...
function notApplicable(ruleId, domain, what) {
    return {
        ruleId,
        domain,
        passed: true,
...
exports.DARK_MODE_RULES = [
    {
        ...
        checkFunction: (ctx) => {
            if (!hasMarkup(ctx))
                return notApplicable('DARKMODE_001', 'color', 'markup');
            const lower = lowerHaystack(ctx);
            if (!hasDarkModeIntent(lower))
                return notApplicable('DARKMODE_001', 'color', 'dark-mode intent');
    ...
    },
    {
        ...
        checkFunction: (ctx) => {
            if (!hasMarkup(ctx))
                return notApplicable('DARKMODE_002', 'color', 'markup');
            const lower = lowerHaystack(ctx);
            if (!hasDarkModeIntent(lower))
                return notApplicable('DARKMODE_002', 'color', 'dark-mode intent');
    ...
    },
    {
        ...
        checkFunction: (ctx) => {
            if (!hasMarkup(ctx))
                return notApplicable('DARKMODE_003', 'color', 'markup');
            const lower = lowerHaystack(ctx);
            if (!/<select\b/.test(lower))
                return notApplicable('DARKMODE_003', 'color', '<select> element');
...
exports.CHART_SELECTION_MATRIX = {
    bar: {
        type: 'bar',
...
function recommendChart(intent) {
    const q = (intent || '').toLowerCase();
    const scored = Object.values(exports.CHART_SELECTION_MATRIX).map((g) => {
        let score = 0;
    ...
    return top.length ? top : [exports.CHART_SELECTION_MATRIX.bar, exports.CHART_SELECTION_MATRIX.line];
}
/** Detect that the markup actually renders a chart. */
function hasChart(lower) {
    return /<svg\b|<canvas\b|class=["'][^"']*\bchart\b|recharts|chart\.js|role=["']img["'][^>]*chart/.test(lower);
}
exports.CHART_SELECTION_RULES = [
    {
        id: 'CHART_001',
        ...
        checkFunction: (ctx) => {
            if (!hasMarkup(ctx))
                return notApplicable('CHART_001', 'data-visualization', 'markup');
            const lower = lowerHaystack(ctx);
            if (!hasChart(lower))
                return notApplicable('CHART_001', 'data-visualization', 'chart');
            const declared = ctx.visualization?.chartType;
            const detail = Object.values(exports.CHART_SELECTION_MATRIX)
    ...
    },
    {
        ...
        checkFunction: (ctx) => {
            if (!hasMarkup(ctx))
                return notApplicable('CHART_002', 'data-visualization', 'markup');
            const lower = lowerHaystack(ctx);
            if (!hasChart(lower))
                return notApplicable('CHART_002', 'data-visualization', 'chart');
    ...
    },
    {
        ...
        checkFunction: (ctx) => {
            if (!hasMarkup(ctx))
                return notApplicable('CHART_003', 'data-visualization', 'markup');
            const lower = lowerHaystack(ctx);
            if (!hasChart(lower))
                return notApplicable('CHART_003', 'data-visualization', 'chart');
    ...
    },
    {
        ...
        checkFunction: (ctx) => {
            if (!hasMarkup(ctx))
                return notApplicable('CHART_004', 'data-visualization', 'markup');
            const lower = lowerHaystack(ctx);
            if (!hasChart(lower))
                return notApplicable('CHART_004', 'data-visualization', 'chart');
...
exports.TIER2_MOTION_RULES = [
    {
        ...
        checkFunction: (ctx) => {
            if (!hasMarkup(ctx))
                return notApplicable('MOTION_HF_001', 'motion', 'markup');
            const lower = lowerHaystack(ctx);
            if (!hasEphemeralUi(lower))
                return notApplicable('MOTION_HF_001', 'motion', 'ephemeral UI (hover/popover/tooltip)');
    ...
    },
    {
        ...
        checkFunction: (ctx) => {
            if (!hasMarkup(ctx))
                return notApplicable('MOTION_PAIR_001', 'motion', 'markup');
            const lower = lowerHaystack(ctx);
            ...
            if (!activePairs.length)
                return notApplicable('MOTION_PAIR_001', 'motion', 'paired animated elements');
            const mismatches = [];
...
// ===========================================================================
exports.CHAR_SUBSTITUTION_RULES = [
    {
        id: 'CHARSUB_001',
        domain: 'typography',
        ...
        checkFunction: (ctx) => {
            if (!hasMarkup(ctx))
                return notApplicable('CHARSUB_001', 'typography', 'markup');
            const text = visibleText(ctx);
            if (!text.trim())
                return notApplicable('CHARSUB_001', 'typography', 'visible text');
    ...
    },
    {
        ...
        checkFunction: (ctx) => {
            if (!hasMarkup(ctx))
                return notApplicable('CHARSUB_002', 'typography', 'markup');
            const text = visibleText(ctx);
            if (!text.trim())
                return notApplicable('CHARSUB_002', 'typography', 'visible text');
            const passed = !/"/.test(text);
            return {
                ruleId: 'CHARSUB_002',
                domain: 'typography',
    ...
    },
    {
        ...
        checkFunction: (ctx) => {
            if (!hasMarkup(ctx))
                return notApplicable('CHARSUB_003', 'typography', 'markup');
    ...
    },
    {
        ...
        checkFunction: (ctx) => {
            if (!hasMarkup(ctx))
                return notApplicable('CHARSUB_004', 'typography', 'markup');
            const lower = lowerHaystack(ctx);
            const hasHeadingAnchor = /<h[1-6][^>]*\bid=/.test(lower);
            if (!hasHeadingAnchor)
                return notApplicable('CHARSUB_004', 'typography', 'heading anchors (h1-h6 with id)');
...
exports.COPYWRITING_RULES = [
    {
        ...
        checkFunction: (ctx) => {
            if (!hasMarkup(ctx))
                return notApplicable('COPY_001', 'ux-writing', 'markup');
            const text = visibleText(ctx);
            if (!text.trim())
                return notApplicable('COPY_001', 'ux-writing', 'visible text');
    ...
    },
    {
        ...
        checkFunction: (ctx) => {
            if (!hasMarkup(ctx))
                return notApplicable('COPY_002', 'ux-writing', 'markup');
            const labels = [...elementTexts(ctx, 'button'), ...elementTexts(ctx, 'h1'), ...elementTexts(ctx, 'h2'), ...elementTexts(ctx, 'h3')];
            if (!labels.length)
                return notApplicable('COPY_002', 'ux-writing', 'button/heading text');
    ...
    },
    {
        ...
        checkFunction: (ctx) => {
            if (!hasMarkup(ctx))
                return notApplicable('COPY_003', 'ux-writing', 'markup');
            const labels = elementTexts(ctx, 'button');
            if (!labels.length)
                return notApplicable('COPY_003', 'ux-writing', 'button text');
    ...
    },
    {
        ...
        checkFunction: (ctx) => {
            if (!hasMarkup(ctx))
                return notApplicable('COPY_004', 'ux-writing', 'markup');
            const lower = lowerHaystack(ctx);
            ...
            if (!errorTexts.length)
                return notApplicable('COPY_004', 'ux-writing', 'error message text');
    ...
    },
    {
        ...
        checkFunction: (ctx) => {
            if (!hasMarkup(ctx))
                return notApplicable('COPY_005', 'ux-writing', 'markup');
            const text = visibleText(ctx);
            if (!text.trim())
                return notApplicable('COPY_005', 'ux-writing', 'visible text');
...
// ===========================================================================
exports.TIER2_VISUAL_COPY_RULES = [
    ...exports.DARK_MODE_RULES,
    ...exports.CHART_SELECTION_RULES,
    ...exports.TIER2_MOTION_RULES,
    ...exports.CHAR_SUBSTITUTION_RULES,
...
//# sourceMappingURL=tier2-visual-copy.js.map