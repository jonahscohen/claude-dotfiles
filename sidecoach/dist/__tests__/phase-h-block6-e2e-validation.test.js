...
function testE2EValidatorRegistrationAndExecution() {
    ...
    const brandRules = [
        flow_composition_1.FlowCompositionEngine.createValidationRule('has_brand_guidance', 'Result includes brand verification guidance', (result) => (result.guidance && result.guidance.some(g => g.includes('brand')))),
        flow_composition_1.FlowCompositionEngine.createValidationRule('has_design_law', 'Result includes design law selection', (result) => (result.guidance && result.guidance.some(g => g.includes('design law')))),
    ];
    const brandValidator = flow_composition_1.FlowCompositionEngine.createDomainValidator('brand_domain', brandRules);
    engine.registerDomainValidator(brandValidator);
    // Step 2: Execute validator on a flow result
    const validResult = createTestResult('flowA_brand_verify', 'success', ['brand verification complete', 'design law selected']);
...
function testCompositeFlowValidationConfigStoresResults() {
    const engine = new flow_composition_1.FlowCompositionEngine();
    // Register validator
    const rule = flow_composition_1.FlowCompositionEngine.createValidationRule('has_content', 'Has content', (result) => (result.guidance && result.guidance.length > 0));
    ...
    const result = createTestResult('flowB_component_research', 'success', ['Research completed']);
...
function testValidationFailureHaltsCompositionWhenConfigured() {
    ...
    const strictRule = flow_composition_1.FlowCompositionEngine.createValidationRule('required_artifacts', 'Must have artifacts', (result) => (result.artifacts && result.artifacts.length > 0));
    const validator = flow_composition_1.FlowCompositionEngine.createDomainValidator('strict_domain', [strictRule]);
    engine.registerDomainValidator(validator);
    ...
    const result = createTestResult('flowC_font_research', 'success', ['Fonts researched'], []);
    const validations = engine.validateMultipleDomains(step.domainValidation.domains, result);
    result.validationResults = validations;
    // Check if validation failed
    const validationFailed = !flow_composition_1.FlowCompositionEngine.allValidationsPassed(validations);
...
function testMultipleValidatorsInSequence() {
    ...
    const accessibility = flow_composition_1.FlowCompositionEngine.createDomainValidator('accessibility', [
        flow_composition_1.FlowCompositionEngine.createValidationRule('wcag', 'WCAG compliance', (result) => (result.guidance && result.guidance.length > 0)),
    ]);
    const performance = flow_composition_1.FlowCompositionEngine.createDomainValidator('performance', [
        flow_composition_1.FlowCompositionEngine.createValidationRule('metrics', 'Performance metrics', (result) => (result.checklist && result.checklist.length > 0)),
    ]);
    const design = flow_composition_1.FlowCompositionEngine.createDomainValidator('design', [
        flow_composition_1.FlowCompositionEngine.createValidationRule('tokens', 'Design tokens', () => true // Always pass
        ),
    ]);
    ...
    const result = createTestResult('flowI_accessibility', 'success', ['WCAG AA compliance verified'], [{ task: 'Performance check' }]);
...
function testMixedValidationResultsPartial() {
    ...
    const complexRule1 = flow_composition_1.FlowCompositionEngine.createValidationRule('rule1', 'Rule 1 - Pass', () => true // Pass
    );
    const complexRule2 = flow_composition_1.FlowCompositionEngine.createValidationRule('rule2', 'Rule 2 - Fail', () => false // Fail
    );
    const complexRule3 = flow_composition_1.FlowCompositionEngine.createValidationRule('rule3', 'Rule 3 - Pass', () => true // Pass
    );
    const validator = flow_composition_1.FlowCompositionEngine.createDomainValidator('complex_domain', [
        complexRule1,
        complexRule2,
        complexRule3,
    ]);
    engine.registerDomainValidator(validator);
    const result = createTestResult('flowD_reference_inspiration');
...
function testValidationContextFlowsThrough() {
    ...
    for (const domain of domains) {
        const rule = flow_composition_1.FlowCompositionEngine.createValidationRule(`rule_${domain}`, `Rule for ${domain}`, () => true);
        const validator = flow_composition_1.FlowCompositionEngine.createDomainValidator(domain, [rule]);
        engine.registerDomainValidator(validator);
    }
    // Execute all validations on same result
    const result = createTestResult('flowE_motion_patterns');
    ...
    const test2 = result.validationResults.every((v) => v.status === 'pass');
    const test3 = result.validationResults.map((v) => v.domain).sort().join(',') === 'domain1,domain2,domain3';
    results.push({
...
function testOrchestratorDecisionPointsBasedOnValidation() {
    ...
    const rule = flow_composition_1.FlowCompositionEngine.createValidationRule('decision_rule', 'Determines next step', (result) => (result.guidance && result.guidance.length > 0));
    const validator = flow_composition_1.FlowCompositionEngine.createDomainValidator('decision_domain', [rule]);
    engine.registerDomainValidator(validator);
    // Scenario 1: Validation passes - continue composition
    const passingResult = createTestResult('flowF_design_tokens', 'success', ['Tokens generated']);
    const passingValidation = engine.validateResult('decision_domain', passingResult);
    const shouldContinue = flow_composition_1.FlowCompositionEngine.allValidationsPassed([passingValidation]);
    // Scenario 2: Validation fails - halt composition
    const failingResult = createTestResult('flowG_component_implementation', 'success', []);
    const failingValidation = engine.validateResult('decision_domain', failingResult);
    const shouldHalt = !flow_composition_1.FlowCompositionEngine.allValidationsPassed([failingValidation]);
    results.push({
...
function testValidationResultsPreservedThroughComposition() {
    const engine = new flow_composition_1.FlowCompositionEngine();
    // Register validators
    const rule1 = flow_composition_1.FlowCompositionEngine.createValidationRule('test1', 'Test 1', () => true);
    const rule2 = flow_composition_1.FlowCompositionEngine.createValidationRule('test2', 'Test 2', () => false);
    const validator = flow_composition_1.FlowCompositionEngine.createDomainValidator('preservation_domain', [rule1, rule2]);
    engine.registerDomainValidator(validator);
    // Execute three flows in sequence, tracking validation
    const results_array = [];
    const result1 = createTestResult('flowH_motion_integration');
    const val1 = engine.validateResult('preservation_domain', result1);
    result1.validationResults = [val1];
    results_array.push(result1);
    const result2 = createTestResult('flowI_accessibility');
    const val2 = engine.validateResult('preservation_domain', result2);
    result2.validationResults = [val2];
    results_array.push(result2);
    const result3 = createTestResult('flowJ_tactical_polish');
...
runTests();
//# sourceMappingURL=phase-h-block6-e2e-validation.test.js.map