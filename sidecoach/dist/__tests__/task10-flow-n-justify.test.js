...
orchestrator.process('/sidecoach rapid button states').then((result) => {
    ...
    const noJustifyMsg = result.flowResults && result.flowResults.some((r) => r.guidance && r.guidance.some((g) => g.includes('DESIGN.md tokens')));
    ...
    const hasFlowN = result.flowResults && result.flowResults.some((r) => r.flowId === 'flowN_rapid_iteration_refined');
    const justifyMsg = result.flowResults && result.flowResults.some((r) => r.message && r.message.includes('Justify'));
    const justifyGuidance = result.flowResults && result.flowResults.some((r) => r.guidance && r.guidance.some((g) => g.includes('LIVE BROWSER ITERATION ENABLED')));
    const justifyArtifact = result.flowResults && result.flowResults.some((r) => r.artifacts && r.artifacts.some((a) => a.name === 'justify-iteration-session'));
    console.log(`  Flow N executed: ${hasFlowN ? 'YES' : 'NO'}`);
    ...
    const artifact = flowN && flowN.artifacts && flowN.artifacts.find((a) => a.name === 'justify-iteration-session');
    if (artifact) {
...
//# sourceMappingURL=task10-flow-n-justify.test.js.map