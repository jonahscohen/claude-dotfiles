...
function createTestResult(flowId, status = 'success', guidance = [], checklist = []) {
    return {
        ...
        guidance,
        checklist,
        artifacts: status === 'success' ? [{ id: `${flowId}_artifact`, name: 'artifact', content: 'content', type: 'reference' }] : [],
    };
}
// Test 1: Basic result injection
function testBasicResultInjection() {
    const context = createTestContext();
    const result = createTestResult('flowA_brand_verify', 'success', ['Design verified'], []);
    const config = {
        injectFlowId: true,
        injectFlowName: true,
        injectGuidance: true,
    };
    const injected = flow_composition_1.FlowCompositionEngine.injectResultIntoContext(context, result, config);
...
function testCustomInjectionKey() {
    const context = createTestContext();
    const result = createTestResult('flowB_component_research');
    const config = {
        injectFlowId: true,
...
function testMetricsInjection() {
    const context = createTestContext();
    const result = createTestResult('flowC_font_research', 'success');
    const config = {
        injectMetrics: true,
    };
    const injected = flow_composition_1.FlowCompositionEngine.injectResultIntoContext(context, result, config);
...
function testIntelligentContextTransform() {
    const context = createTestContext();
    const result = createTestResult('flowD_reference_inspiration', 'success', ['Reference found', 'Pattern detected'], [{ task: 'Review references' }]);
    const transformed = flow_composition_1.FlowCompositionEngine.intelligentContextTransform(context, result);
    ...
    results.push({
        ...
        passed: hasPreviousFlowId && hasGuidanceArray && hasGuidanceCount && hasChecklistCount,
        message: 'All result properties intelligently injected',
    });
}
// Test 5: Conditional transformer
function testConditionalTransformer() {
    const shouldTransform = (result) => result.status === 'success';
    const transform = (context, result) => {
    ...
    const transformer = flow_composition_1.FlowCompositionEngine.createConditionalTransformer(shouldTransform, transform);
    const context = createTestContext();
    const successResult = createTestResult('flowE_motion_patterns', 'success');
    const errorResult = createTestResult('flowF_design_tokens', 'error');
...
function testResultAccumulation() {
    const context = createTestContext();
    const results1 = [
        createTestResult('flowA_brand_verify', 'success'),
        createTestResult('flowB_component_research', 'success'),
    ];
    const results2 = [
        createTestResult('flowC_font_research', 'error'),
    ];
    let accumulated = flow_composition_1.FlowCompositionEngine.accumulateResultsInContext(context, results1);
    accumulated = flow_composition_1.FlowCompositionEngine.accumulateResultsInContext(accumulated, results2);
...
function testArtifactInjection() {
    const context = createTestContext();
    const result = createTestResult('flowG_component_implementation', 'success');
    result.artifacts = [
...
function testChecklistInjection() {
    const context = createTestContext();
    const result = createTestResult('flowH_motion_integration', 'success');
    result.checklist = [
...
function testNoInjectionWhenEmpty() {
    const context = createTestContext();
    const result = createTestResult('flowI_accessibility');
    const config = {};
    const injected = flow_composition_1.FlowCompositionEngine.injectResultIntoContext(context, result, config);
...
function testMultipleTransformations() {
    ...
    const result1 = createTestResult('flowJ_tactical_polish', 'success', ['Polish applied']);
    context = flow_composition_1.FlowCompositionEngine.intelligentContextTransform(context, result1);
    const result2 = createTestResult('flowK_multi_lens_audit', 'success', ['Issues found']);
    context = flow_composition_1.FlowCompositionEngine.intelligentContextTransform(context, result2);
...
runTests();
//# sourceMappingURL=phase-h-block3-result-injection.test.js.map