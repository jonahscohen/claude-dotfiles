...
class SidecoachOrchestrator {
    ...
    detectPhase(context) {
        ...
        const ranResearch = researchFlows.some((f) => sequence.some((h) => h.flowId === f));
        if (!ranResearch)
            return 'research';
        ...
        const ranExecution = executionFlows.some((f) => sequence.some((h) => h.flowId === f));
        if (!ranExecution)
            return 'research';
        ...
        const ranQA = qaFlows.some((f) => sequence.some((h) => h.flowId === f));
        if (!ranQA)
            return 'execution';
    ...
    validatePrerequisites(flowId) {
        ...
        const sequence = this.history.getFlowSequence();
        const missingFlows = dep.prerequisiteFlows.filter((f) => !sequence.some((h) => h.flowId === f && h.status === 'success'));
    ...
    getNextRecommendedFlow(currentFlowId, currentResult) {
    ...
    getWorkflowRecommendation(context) {
        ...
        const nextFlow = sequence.find((f) => !history.some((h) => h.flowId === f && h.status === 'success'));
        const completed = sequence.filter((f) => history.some((h) => h.flowId === f && h.status === 'success')).length;
        return {
            phase,
...
function createOrchestrator(history) {
