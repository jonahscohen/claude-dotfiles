...
exports.createReferenceDataService = createReferenceDataService;
...
element.addEventListener("mouseleave", () => {
  ...
  onMouseDown: true,
                ...
                for (const line of lines) {
                    ...
                    if (!line.startsWith(' ') && line.includes(':')) {
                        if (currentSection && Object.keys(currentNested).length > 0) {
                            tokens[currentSection] = currentNested;
                        }
    ...
    // PUBLIC API
    searchComponents(query, personality) {
        const results = [];
    ...
    searchFonts(personality) {
        const results = [];
        for (const font of Object.values(this.fontCatalog)) {
            if (font.personality.some((p) => p.toLowerCase().includes(personality.toLowerCase()))) {
                results.push(font);
            }
        }
        return results.slice(0, 5);
    ...
    searchMotionPatterns(category) {
        return Object.values(this.motionPatterns).filter((pattern) => pattern.category.toLowerCase().includes(category.toLowerCase()) ||
            pattern.name.toLowerCase().includes(category.toLowerCase()));
    }
    getMotionPattern(name) {
        return this.motionPatterns[name.toLowerCase().replace(/\s+/g, '_')];
    }
    getAllMotionPatterns() {
    ...
    searchDesignReferences(query) {
        const results = [];
        const lowerQuery = query.toLowerCase();
        for (const ref of Object.values(this.designReferences)) {
            if (ref.id.includes(lowerQuery) ||
    ...
    getDesignReference(id) {
        return this.designReferences[id];
    }
    getDesignReferencesByCategory(category) {
        return Object.values(this.designReferences).filter((ref) => ref.category.toLowerCase().includes(category.toLowerCase()));
    }
    getDesignReferencesByFeel(feel) {
        return Object.values(this.designReferences).filter((ref) => ref.feel.some((f) => f.toLowerCase().includes(feel.toLowerCase())));
    }
    getAllDesignReferences() {
        return Object.values(this.designReferences);
    }
    getDesignTokens() {
        return this.designTokens;
    }
    getDesignTokensBySection(section) {
        return this.designTokens[section] || {};
    }
    getDesignTokenValue(path) {
        const parts = path.split('.');
        let current = this.designTokens;
...
.${componentName}:focus-visible {
  ...
  outline-offset: 2px;
    ...
    generateMotionTemplate(patternName) {
...
function createReferenceDataService() {
