...
function formsHaystack(ctx: DomainCheckContext): string {
  const parts: string[] = [];
...
function hasFormMarkup(ctx: DomainCheckContext): boolean {
...
function gestureHaystack(ctx: DomainCheckContext): string {
  const parts: string[] = [];
  if (ctx.html) parts.push(ctx.html);
  if (ctx.cssRules?.length) parts.push(ctx.cssRules.join('\n'));
...
function hasDragInteraction(ctx: DomainCheckContext): boolean {
...
const DOMAIN_RULES: DomainValidationRule[] = [
...

  {
    ...
    severity: 'critical',
    checkFunction: (ctx) => {
      if (!ctx.htmlElement) return { ruleId: 'POLISH_005', domain: 'polish', passed: false, message: 'Cannot measure element' };
...

  {
    ...
    checkFunction: (ctx) => ({
      ruleId: 'POLISH_007',
      domain: 'polish',
      passed: !!(ctx.computedStyle && (ctx.computedStyle as any).fontVariantNumeric?.includes('tabular')),
...

  {
    ...
    severity: 'critical',
    checkFunction: (ctx) => {
      if (!ctx.htmlElement) return { ruleId: 'SPACE_014', domain: 'spatial', passed: false, message: 'Cannot measure element' };
      const rect = ctx.htmlElement.getBoundingClientRect();
      return {
        ruleId: 'SPACE_014',
        domain: 'spatial',
...

  {
    ...
    severity: 'critical',
    checkFunction: (ctx) => {
      if (!ctx.htmlElement) return { ruleId: 'RESPOND_003', domain: 'responsive', passed: false, message: 'Cannot measure element' };
      const rect = ctx.htmlElement.getBoundingClientRect();
      return {
        ruleId: 'RESPOND_003',
        domain: 'responsive',
...

  {
    ...
    severity: 'high',
    checkFunction: (ctx) => {
      if (!hasFormMarkup(ctx)) return { ruleId: 'FORMS_001', domain: 'forms', passed: true, message: 'No form markup detected (rule N/A)' };
...

  {
    ...
    severity: 'high',
    checkFunction: (ctx) => {
      if (!hasFormMarkup(ctx)) return { ruleId: 'FORMS_002', domain: 'forms', passed: true, message: 'No form markup detected (rule N/A)' };
      const h = formsHaystack(ctx);
...

  {
    ...
    severity: 'medium',
    checkFunction: (ctx) => {
      if (!hasFormMarkup(ctx)) return { ruleId: 'FORMS_003', domain: 'forms', passed: true, message: 'No form markup detected (rule N/A)' };
      const h = formsHaystack(ctx);
...

  {
    ...
    severity: 'high',
    checkFunction: (ctx) => {
      if (!hasFormMarkup(ctx)) return { ruleId: 'FORMS_004', domain: 'forms', passed: true, message: 'No form markup detected (rule N/A)' };
      const h = formsHaystack(ctx);
...

  {
    ...
    severity: 'medium',
    checkFunction: (ctx) => {
      if (!hasFormMarkup(ctx)) return { ruleId: 'FORMS_005', domain: 'forms', passed: true, message: 'No form markup detected (rule N/A)' };
      const h = formsHaystack(ctx);
...

  {
    ...
    severity: 'high',
    checkFunction: (ctx) => {
      if (!hasFormMarkup(ctx)) return { ruleId: 'FORMS_006', domain: 'forms', passed: true, message: 'No form markup detected (rule N/A)' };
      const h = formsHaystack(ctx);
...

  {
    ...
    severity: 'medium',
    checkFunction: (ctx) => {
      if (!hasFormMarkup(ctx)) return { ruleId: 'FORMS_007', domain: 'forms', passed: true, message: 'No form markup detected (rule N/A)' };
      const h = formsHaystack(ctx);
...

  {
    ...
    severity: 'high',
    checkFunction: (ctx) => {
      if (!hasFormMarkup(ctx)) return { ruleId: 'FORMS_008', domain: 'forms', passed: true, message: 'No form markup detected (rule N/A)' };
      const h = formsHaystack(ctx);
...

  {
    ...
    severity: 'high',
    checkFunction: (ctx) => {
      if (!hasFormMarkup(ctx)) return { ruleId: 'FORMS_009', domain: 'forms', passed: true, message: 'No form markup detected (rule N/A)' };
      const h = formsHaystack(ctx);
...

  {
    ...
    severity: 'low',
    checkFunction: (ctx) => {
      if (!hasFormMarkup(ctx)) return { ruleId: 'FORMS_010', domain: 'forms', passed: true, message: 'No form markup detected (rule N/A)' };
      const h = formsHaystack(ctx);
...

  {
    ...
    severity: 'medium',
    checkFunction: (ctx) => {
      if (!hasFormMarkup(ctx)) return { ruleId: 'FORMS_011', domain: 'forms', passed: true, message: 'No form markup detected (rule N/A)' };
      const h = formsHaystack(ctx);
...

  {
    ...
    severity: 'medium',
    checkFunction: (ctx) => {
      if (!hasFormMarkup(ctx)) return { ruleId: 'FORMS_012', domain: 'forms', passed: true, message: 'No form markup detected (rule N/A)' };
      const h = gestureHaystack(ctx);
...

  {
    ...
    severity: 'medium',
    checkFunction: (ctx) => {
      if (!hasFormMarkup(ctx)) return { ruleId: 'FORMS_013', domain: 'forms', passed: true, message: 'No form markup detected (rule N/A)' };
      const h = gestureHaystack(ctx);
...

  {
    ...
    severity: 'medium',
    checkFunction: (ctx) => {
      if (!hasFormMarkup(ctx)) return { ruleId: 'FORMS_014', domain: 'forms', passed: true, message: 'No form markup detected (rule N/A)' };
      const h = gestureHaystack(ctx);
...

  {
    ...
    severity: 'high',
    checkFunction: (ctx) => {
      if (!hasFormMarkup(ctx)) return { ruleId: 'FORMS_015', domain: 'forms', passed: true, message: 'No form markup detected (rule N/A)' };
      const h = formsHaystack(ctx);
...

  {
    ...
    severity: 'critical',
    checkFunction: (ctx) => {
      if (!hasFormMarkup(ctx)) return { ruleId: 'FORMS_016', domain: 'forms', passed: true, message: 'No form markup detected (rule N/A)' };
      const h = formsHaystack(ctx);
...

  {
    ...
    severity: 'medium',
    checkFunction: (ctx) => {
      if (!hasFormMarkup(ctx)) return { ruleId: 'FORMS_017', domain: 'forms', passed: true, message: 'No form markup detected (rule N/A)' };
      const h = formsHaystack(ctx);
...

  {
    ...
    severity: 'high',
    checkFunction: (ctx) => {
      if (!hasFormMarkup(ctx)) return { ruleId: 'FORMS_018', domain: 'forms', passed: true, message: 'No form markup detected (rule N/A)' };
      const h = formsHaystack(ctx);
...

  {
    ...
    severity: 'high',
    checkFunction: (ctx) => {
      if (!hasFormMarkup(ctx)) return { ruleId: 'FORMS_019', domain: 'forms', passed: true, message: 'No form markup detected (rule N/A)' };
      const h = formsHaystack(ctx);
      const hasPlaceholder = h.includes('placeholder');
...

  {
    ...
    severity: 'low',
    checkFunction: (ctx) => {
      if (!hasFormMarkup(ctx)) return { ruleId: 'FORMS_020', domain: 'forms', passed: true, message: 'No form markup detected (rule N/A)' };
      const h = formsHaystack(ctx);
