...
export interface ValidateTasteOptions {
  iconLibrary?: string;
  // T-0032: explicit Tailwind/shadcn signal for callers that have filesystem
  // access (e.g. a detected components.json). When true, the token-utility
  // carve-outs below are enabled regardless of the html/css content heuristics.
...
const TAILWIND_DIRECTIVE_PATTERN =
  /@tailwind\b|@apply\b|tailwind\.config|(?:hsl|rgb|oklch)\(\s*var\(\s*--/i;
...
function detectTailwindContext(
  ...
  // T-0032
  if (opts?.componentsJson) return true;
  ...
  for (const m of html.matchAll(classAttrRe)) {
    if (TAILWIND_UTILITY_PATTERN.test(m[1])) return true;
  }
  return false;
}
...
function blockReferencesToken(body: string, tailwind: boolean): boolean {
  // T-0032
  if (/var\(\s*--[\w-]+/.test(body)) return true;
  if (tailwind && TAILWIND_UTILITY_PATTERN.test(body)) return true;
  return false;
}
...
function* iterateCssBlocks(
  css: string
): Generator<{ selector: string; body: string }> {
  let i = 0;
  while (i < css.length) {
...
function checkTranslateYInHover(allCss: string): TasteViolation[] {
  const violations: TasteViolation[] = [];
  for (const block of iterateCssBlocks(allCss)) {
    if (!/:hover\b/.test(block.selector)) continue;
    if (!/transform\s*:[^;]*translateY\s*\(/.test(block.body)) continue;
    violations.push({
...
function checkLargeInlineStyle(html: string): TasteViolation[] {
  const violations: TasteViolation[] = [];
  const headMatch = html.match(/<head\b[^>]*>([\s\S]*?)<\/head>/i);
  if (!headMatch || headMatch.index === undefined) return violations;
...
function checkHeroRadialGradient(allCss: string): TasteViolation[] {
  const violations: TasteViolation[] = [];
  const heroSelectorRe =
    /(^|\s|,)(\.hero|\.banner|\[class\*=["']hero["']\]|\[id\*=["']hero["']\])(\b|[:.])/i;
  for (const block of iterateCssBlocks(allCss)) {
    if (!heroSelectorRe.test(block.selector)) continue;
    if (!/radial-gradient\s*\(/.test(block.body)) continue;
    violations.push({
...
function checkHexInHoverWithCssVars(allCss: string, tailwind: boolean): TasteViolation[] {
  ...
  if (!fileHasCssVars) return violations;
  for (const block of iterateCssBlocks(allCss)) {
    if (!/(:hover|:active)\b/.test(block.selector)) continue;
    ...
    if (tailwind && blockReferencesToken(block.body, tailwind)) continue;
...
function checkObserverRace(html: string, allCss: string): TasteViolation[] {
  const violations: TasteViolation[] = [];
  ...
  if (!usesObserver) return violations;
  ...
  if (observedClasses.size === 0) return violations;

  const offending: string[] = [];
  for (const cls of observedClasses) {
    const re = new RegExp(`\\.${cls}\\b[^{]*\\{[^}]*opacity\\s*:\\s*0\\b`, 'i');
    if (re.test(allCss)) offending.push('.' + cls);
  }
  if (offending.length === 0) return violations;
...
function checkBorderRadiusInconsistency(allCss: string, tailwind: boolean): TasteViolation[] {
  ...
  for (const m of radiusMatches) {
    ...
    if (tailwind && /var\(\s*--[\w-]+/.test(v)) continue;
    values.add(v);
  }
  if (values.size > 2) {
    violations.push({
...
export function formatViolations(violations: TasteViolation[], filePath: string): string {
