...
export const DOMAIN_FLOW_MAP: DomainFlowMapping[] = [
  {
    domain: 'typography',
...
export const FLOW_DOMAIN_MATRIX: FlowDomainRequirement[] = [
  {
...
export function getDomainsForFlow(flowId: FlowId): string[] {
  const requirement = FLOW_DOMAIN_MATRIX.find(f => f.flowId === flowId);
  return requirement?.domains || [];
}

export function getPrimaryDomainsForFlow(flowId: FlowId): string[] {
  const requirement = FLOW_DOMAIN_MATRIX.find(f => f.flowId === flowId);
  return requirement?.primaryDomains || [];
}

export function getFlowsForDomain(domain: string): FlowId[] {
  const mapping = DOMAIN_FLOW_MAP.find(d => d.domain === domain);
  return mapping?.flowIds || [];
}

export function getValidationPointsForDomain(domain: string): string[] {
  const mapping = DOMAIN_FLOW_MAP.find(d => d.domain === domain);
