...
export interface SlopDetectionResult {
  ...
  genericityScore: number; // 0-100 (0=unique, 100=generic)
...
export class CategoryReflexDetector {
  /**
  ...
  private matchPatternSignature(reference: DesignReference, patterns: string[]): number {
    let matches = 0;
  ...
  private analyzeDescriptionGenericity(reference: DesignReference): number {
    const genericTerms = [
      'modern',
      'clean',
      'minimal',
  ...
  private determineVerdict(score: number): 'keep' | 'flag' | 'discard' {
    if (score >= 80) return 'discard'; // Highly generic
    if (score >= 60) return 'flag'; // Moderately generic, use cautiously
...
export function createCategoryReflexDetector(): CategoryReflexDetector {
