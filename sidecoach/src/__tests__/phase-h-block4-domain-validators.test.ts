...
import { FlowCompositionEngine, ValidationRule, DomainValidator, ValidationResult } from '../flow-composition';
...
function testCreateValidationRule() {
  const rule = FlowCompositionEngine.createValidationRule(
    'has_guidance',
    'Result must include guidance',
    (result) => (result.guidance && result.guidance.length > 0) as boolean
  );

  const test1: boolean = rule.name === 'has_guidance';
  const test2: boolean = rule.description === 'Result must include guidance';
  const test3: boolean = typeof rule.validate === 'function';
...
function testCreateDomainValidator() {
  const rule1 = FlowCompositionEngine.createValidationRule('rule1', 'Test rule 1', () => true);
  const rule2 = FlowCompositionEngine.createValidationRule('rule2', 'Test rule 2', () => true);

  const validator = FlowCompositionEngine.createDomainValidator('accessibility', [rule1, rule2], false);

  const test1: boolean = validator.domain === 'accessibility';
  const test2: boolean = validator.rules.length === 2;
  const test3: boolean = (validator.failOnFirstError ?? false) === false;
...
function testRegisterAndRetrieveValidator() {
  ...
  const rule = FlowCompositionEngine.createValidationRule('test', 'test', () => true);
  const validator = FlowCompositionEngine.createDomainValidator('performance', [rule]);

  engine.registerDomainValidator(validator);
  const retrieved = engine.getDomainValidator('performance');

  const test1: boolean = retrieved !== null;
  const test2: boolean = (retrieved?.domain ?? '') === 'performance';
  const test3: boolean = (retrieved?.rules?.length ?? 0) === 1;
...
function testValidateResultPass() {
  ...
  const validator = FlowCompositionEngine.createDomainValidator('accessibility', [rule1, rule2]);
  engine.registerDomainValidator(validator);
  ...
  const test1: boolean = validation.status === 'pass';
  const test2: boolean = (validation.passedRules?.length ?? 0) === 2;
  const test3: boolean = (validation.failedRules?.length ?? 0) === 0;
...
function testValidateResultFail() {
  ...
  const result = createTestResult('flowI_accessibility' as FlowId, 'success', [], []);

  const validation = engine.validateResult('accessibility', result);

  const test1: boolean = validation.status === 'fail';
  const test2: boolean = (validation.passedRules?.length ?? 0) === 0;
  const test3: boolean = (validation.failedRules?.length ?? 0) === 2;
...
function testValidateResultPartial() {
  ...
  const rule3 = FlowCompositionEngine.createValidationRule(
    'has_artifacts',
    'Has artifacts',
    (result) => (result.artifacts && result.artifacts.length > 0) as boolean
  );

  const validator = FlowCompositionEngine.createDomainValidator('accessibility', [rule1, rule2, rule3]);
  engine.registerDomainValidator(validator);

  const result = createTestResult('flowI_accessibility' as FlowId, 'success', ['Guidance provided'], []);

  const validation = engine.validateResult('accessibility', result);

  const test1: boolean = validation.status === 'partial';
  const test2: boolean = (validation.passedRules?.length ?? 0) === 1;
  const test3: boolean = (validation.failedRules?.length ?? 0) === 2;
...
function testMultipleDomainValidation() {
  ...
  const test1: boolean = (validations?.length ?? 0) === 2;
  const test2: boolean = (validations?.[0]?.status ?? '') === 'pass';
  const test3: boolean = (validations?.[1]?.status ?? '') === 'pass';
...
function testAllValidationsPassed() {
  ...
  const mixedValidations: any[] = [
    { status: 'pass', domain: 'accessibility' },
    { status: 'partial', domain: 'performance' },
  ];

  const test1: boolean = FlowCompositionEngine.allValidationsPassed(passedValidations) === true;
  const test2: boolean = FlowCompositionEngine.allValidationsPassed(mixedValidations) === false;
...
function testFailOnFirstError() {
  ...
  const rule1 = FlowCompositionEngine.createValidationRule('rule1', 'Rule 1', () => false);
  const rule2 = FlowCompositionEngine.createValidationRule('rule2', 'Rule 2', () => false);

  const validator = FlowCompositionEngine.createDomainValidator('test_domain', [rule1, rule2], true);
  engine.registerDomainValidator(validator);

  const result = createTestResult('flowA_brand_verify' as FlowId);
  const validation = engine.validateResult('test_domain', result);

  const test1: boolean = (validation.failedRules?.length ?? 0) === 1;
  const test2: boolean = validation.status === 'partial';
