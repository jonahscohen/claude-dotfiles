...
} from '../domains/tier2-content-perf';

function fail(msg: string): never { console.error('FAIL: ' + msg); process.exit(1); }
function run(id: string, ctx: DomainCheckContext) {
  const rule = TIER2_CONTENT_PERF_RULES.find(r => r.id === id);
  if (!rule) fail(`rule ${id} not found`);
  return rule!.checkFunction(ctx);
}

// --- 1. unique ids + correct domains + counts -----------------------------
const ids = TIER2_CONTENT_PERF_RULES.map(r => r.id);
if (new Set(ids).size !== ids.length) fail('duplicate rule id detected');
if (ids.length !== 15) fail(`expected 15 rules, got ${ids.length}`);
if (TIER2_CONTENT_RESILIENCE_RULES.length !== 4) fail('content-resilience should have 4 rules');
if (TIER2_TOUCH_RULES.length !== 4) fail('touch should have 4 rules');
if (TIER2_IMAGE_PERF_RULES.length !== 3) fail('image-perf should have 3 rules');
if (TIER2_PERF_SPECIFICS_RULES.length !== 4) fail('perf-specifics should have 4 rules');

for (const r of TIER2_CONTENT_RESILIENCE_RULES) if (r.domain !== 'polish') fail(`${r.id} domain should be polish`);
for (const r of TIER2_TOUCH_RULES) if (r.domain !== 'responsive') fail(`${r.id} domain should be responsive`);
for (const r of [...TIER2_IMAGE_PERF_RULES, ...TIER2_PERF_SPECIFICS_RULES]) if (r.domain !== 'performance') fail(`${r.id} domain should be performance`);

// every result echoes its own ruleId/domain
for (const r of TIER2_CONTENT_PERF_RULES) {
  const res = r.checkFunction({});
  if (res.ruleId !== r.id) fail(`${r.id} returns mismatched ruleId ${res.ruleId}`);
  if (res.domain !== r.domain) fail(`${r.id} returns mismatched domain ${res.domain}`);
}

// --- 2. positive fixtures pass --------------------------------------------
const positive: Record<string, DomainCheckContext> = {
...
for (const [id, ctx] of Object.entries(positive)) {
  ...
  if (!res.passed) fail(`${id} positive fixture should pass -> ${res.message}`);
}

// --- 3. negative fixtures fail --------------------------------------------
const negative: Record<string, DomainCheckContext> = {
...
for (const [id, ctx] of Object.entries(negative)) {
  const res = run(id, ctx);
  if (res.passed) fail(`${id} negative fixture should fail -> ${res.message}`);
}

// --- 4. absent markup => N/A pass -----------------------------------------
for (const r of TIER2_CONTENT_PERF_RULES) {
  const res = r.checkFunction({});
  if (!res.passed) fail(`${r.id} should be N/A pass on empty context -> ${res.message}`);
  if (!/N\/A/.test(res.message)) fail(`${r.id} N/A message should say N/A -> ${res.message}`);
}
