...
orchestrator.process('').then((result: any) => {
  ...
  const hasPhaseHeaders = result.guidance &&
    result.guidance.some((g: string) => g.includes('Research Phase')) &&
    result.guidance.some((g: string) => g.includes('Implement Phase')) &&
    result.guidance.some((g: string) => g.includes('Review Phase')) &&
    result.guidance.some((g: string) => g.includes('Special Phase'));
  ...
  const numericEntries = result.guidance && result.guidance.filter((g: string) => /^\s*\d+\./.test(g)).length;
