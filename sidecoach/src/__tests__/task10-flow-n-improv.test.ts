...
orchestrator.process('/sidecoach rapid button states').then((result: any) => {
  ...
  const noImproveMsg = result.flowResults && result.flowResults.some((r: any) =>
  ...
  const hasFlowN = result.flowResults && result.flowResults.some((r: any) => r.flowId === 'flowN_rapid_iteration_refined');
  const improveMsg = result.flowResults && result.flowResults.some((r: any) =>
    r.message && r.message.includes('Improv')
  );
  const improveGuidance = result.flowResults && result.flowResults.some((r: any) =>
    r.guidance && r.guidance.some((g: string) => g.includes('LIVE BROWSER ITERATION ENABLED'))
  );
  const improveArtifact = result.flowResults && result.flowResults.some((r: any) =>
    r.artifacts && r.artifacts.some((a: any) => a.name === 'improv-iteration-session')
  );
  ...
  const artifact = flowN && flowN.artifacts && flowN.artifacts.find((a: any) => a.name === 'improv-iteration-session');
