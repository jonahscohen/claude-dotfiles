...
async function run() {
  ...
  // --- Scenario 3: --deep with vague users answer surfaces follow-up question ---
  {
    ...
    const result = await handler.execute({ utterance, projectPath: sandbox, metadata: {} } as any);
    checks.push(['S3: --deep with vague answers returns pending', (result.status as string) === 'pending']);
  ...
  {
    const r1 = parseDeepFlag('--deep Register: brand');
    ...
    const r2 = parseDeepFlag('Register: brand');
    checks.push(['S4: default depth is standard', r2.depth === 'standard']);
    const r3 = parseDeepFlag('Register: brand --quick');
    checks.push(['S4: parseDeepFlag detects --quick at end', r3.depth === 'quick']);
  }

  // --- Scenario 5: isVagueAnswer detection ---
  {
    checks.push(['S5: bare "developers" is vague for users', isVagueAnswer('users', 'developers')]);
    checks.push([
      'S5: specific PM role is NOT vague for users',
      !isVagueAnswer('users', 'PMs at mid-sized agencies running daily standup'),
    ]);
    checks.push([
      'S5: "modern" alone is vague for brandPersonality',
      isVagueAnswer('brandPersonality', 'modern'),
    ]);
    checks.push([
      'S5: "ink-on-paper, restrained" is NOT vague for brandPersonality',
      !isVagueAnswer('brandPersonality', 'ink-on-paper, technical, restrained'),
    ]);
    checks.push([
      'S5: empty string is vague',
      isVagueAnswer('successMetrics', ''),
    ]);
  }

  // --- Scenario 6: generateFollowUpQuestion produces a sharper prompt ---
  {
    const q = generateFollowUpQuestion('users', 'developers');
    ...
    const qProblem = generateFollowUpQuestion('problem', 'make something');
  ...
  // --- Scenario 7: ambiguityScore math ---
  {
    ...
    const all = ambiguityScore({}, DEEP_FIELDS);
    ...
    for (const f of DEEP_FIELDS) allHigh[f] = 'high';
    const clean = ambiguityScore(allHigh as any, DEEP_FIELDS);
    ...
    const mixed: Record<string, 'high' | 'absent'> = {};
    for (const f of DEEP_FIELDS) {
      mixed[f] = FIELD_DIMENSION[f] === 'goal' ? 'high' : 'absent';
    }
    const m = ambiguityScore(mixed as any, DEEP_FIELDS);
  ...
  // --- Scenario 8: validateProductMd structural validation ---
  {
    ...
    const v1 = validateProductMd(goodBrand, { register: 'brand', deep: false });
    ...
    const v2 = validateProductMd(missing, { register: 'brand', deep: false });
    ...
    const v3 = validateProductMd(goodBrand, { register: 'brand', deep: true });
    checks.push([
    ...
    const v4 = validateProductMd(withTodo, { register: 'brand', deep: false });
  ...
  {
    checks.push(['S12: CORE_FIELDS has 5 entries', CORE_FIELDS.length === 5]);
    checks.push(['S12: EXTENDED_FIELDS has 5 entries (4 generic + brandVoice)', EXTENDED_FIELDS.length === 5]);
    checks.push(['S12: DEEP_FIELDS has 10 entries total', DEEP_FIELDS.length === 10]);
    checks.push(['S12: every DEEP_FIELD has a dimension mapping', DEEP_FIELDS.every((f) => !!FIELD_DIMENSION[f])]);
  }
