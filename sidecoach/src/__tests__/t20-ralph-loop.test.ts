// T-0020: Ralph-mode relentless cross-flow iteration tests.
//
...
function makeFinding(flowId: FlowId, ruleId: string, validator = 'polish-standard'): RalphFinding {
...
async function run(): Promise<void> {
  // === Section 1: Convergence - validators clean on the first try ===
  {
    ...
    const runFlow: RalphFlowRunner = async () => ({ findings: [] });
    const result = await runRalphLoop('/tmp/clean-target', { runFlow, logger: cap.logger });
    ...
    expect(
      ...
      DEFAULT_RALPH_FLOW_CHAIN.every((flowId) =>
  ...
  // === Section 2: Stalled - same findings repeat for maxNoProgress iterations ===
  {
    ...
    const result = await runRalphLoop('/tmp/stalled-target', {
      runFlow,
      logger: cap.logger,
  ...
  // === Section 3: Capped - findings persist but evolve enough to avoid stall ===
  {
    const cap = makeCaptureLogger();
    let iter = 0;
    const runFlow: RalphFlowRunner = async ({ flowId, iteration }) => {
      iter = iteration;
    ...
    const result = await runRalphLoop('/tmp/capped-target', {
      runFlow,
      logger: cap.logger,
    ...
    expect(
      ...
      result.iterations === DEFAULT_RALPH_MAX_GLOBAL_ITERATIONS,
      String(result.iterations),
    );
    expect(
      'capped: hit exactly DEFAULT_RALPH_MAX_GLOBAL_ITERATIONS (10) iter values',
      iter === DEFAULT_RALPH_MAX_GLOBAL_ITERATIONS,
      String(iter),
    );
    expect(
  ...
  // === Section 4: Progress signature differs each iter -> no stall, no converge ===
  {
    ...
    const result = await runRalphLoop('/tmp/no-stall-target', {
      runFlow,
      logger: cap.logger,
  ...
  // === Section 5: Empty flow chain -> error result ===
  {
    const cap = makeCaptureLogger();
    const runFlow: RalphFlowRunner = async () => ({ findings: [] });
    const result = await runRalphLoop('/tmp/empty-chain-target', {
      runFlow,
      logger: cap.logger,
      flowChain: [],
    });
  ...
  {
    const cap = makeCaptureLogger();
    const result = await runRalphLoop('/tmp/no-runner-target', { logger: cap.logger });
  ...
  // The spec calls for `[ralph] iter N/M: <flow> found <X> violations` lines.
  {
    const cap = makeCaptureLogger();
    const runFlow: RalphFlowRunner = async ({ flowId, iteration }) => {
    ...
    const result = await runRalphLoop('/tmp/format-target', {
      runFlow,
      logger: cap.logger,
      maxGlobalIterations: 5,
    ...
    expect(
      'log-format: iter 1 line matches `[ralph] iter N/M: <flow> found X violations`',
      typeof iter1Line === 'string' && iter1Line.startsWith('[ralph]'),
      iter1Line,
    );
  ...
  // === Section 8: Runner throw is isolated to that flow on that iteration ===
  {
    const cap = makeCaptureLogger();
    let invocations = 0;
    const runFlow: RalphFlowRunner = async ({ flowId }) => {
      invocations++;
      if (flowId === 'flowK_multi_lens_audit') {
        throw new Error('synthetic-runner-failure');
      }
      // Polish + critique find nothing. Audit throws. After convergence
      // path: polish=0, audit=0 (throw counted as zero), critique=0 ->
      // total=0 -> converge.
      return { findings: [] };
    };
    const result = await runRalphLoop('/tmp/runner-throw-target', {
      runFlow,
      logger: cap.logger,
    });
    expect(
      'runner-throw: still converged because findings still total 0',
      result.status === 'converged',
      `${result.status} totalFindings=${result.totalFindings}`,
    );
    expect(
      'runner-throw: all three default-chain flows were invoked',
      invocations === DEFAULT_RALPH_FLOW_CHAIN.length,
      String(invocations),
    );
    expect(
    ...
    const auditRecord = result.history[0]?.flowResults.find(
      (fr) => fr.flowId === 'flowK_multi_lens_audit',
    );
    expect(
  ...
  // === Section 9: Chain ordering preserved in invocation order ===
  {
    ...
    await runRalphLoop('/tmp/order-target', {
      runFlow,
      logger: () => undefined,
  ...
  // === Section 10: applyFixes is invoked between iterations and can affect findings ===
  {
    ...
    const result = await runRalphLoop('/tmp/applyfixes-target', {
      runFlow,
      logger: () => undefined,
      applyFixes,
      maxGlobalIterations: 5,
  ...
  // === Section 12: Defaults match the spec ===
  {
    expect(
      ...
      DEFAULT_RALPH_MAX_GLOBAL_ITERATIONS === 10,
      String(DEFAULT_RALPH_MAX_GLOBAL_ITERATIONS),
    );
    expect(
      'defaults: maxNoProgressIterations=3',
      DEFAULT_RALPH_MAX_NO_PROGRESS_ITERATIONS === 3,
      String(DEFAULT_RALPH_MAX_NO_PROGRESS_ITERATIONS),
    );
    expect(
      'defaults: chain is [polish, audit, critique]',
      DEFAULT_RALPH_FLOW_CHAIN.length === 3 &&
        DEFAULT_RALPH_FLOW_CHAIN[0] === 'flowJ_tactical_polish' &&
        DEFAULT_RALPH_FLOW_CHAIN[1] === 'flowK_multi_lens_audit' &&
        DEFAULT_RALPH_FLOW_CHAIN[2] === 'flowL_design_critique',
      JSON.stringify(DEFAULT_RALPH_FLOW_CHAIN),
    );
  }
  ...
  console.log('---');
  console.log(`t20-ralph-loop: ${passed}/${total} passed`);
  console.log(allPass ? 't20-ralph-loop PASS' : 't20-ralph-loop FAIL');
