...
const dirty = validateTaste(HTML_WITH_OBSERVER);
const clean = validateTaste(HTML_WITHOUT_OBSERVER);

const dirtyHit = dirty.find(v => v.ruleId === 'taste/observer-race');
const cleanHit = clean.find(v => v.ruleId === 'taste/observer-race');

if (!dirtyHit) { console.error('FAIL: observer-race not flagged on observer-driven HTML'); process.exit(1); }
if (cleanHit) { console.error('FAIL: observer-race wrongly flagged on CSS-animation HTML'); process.exit(1); }
