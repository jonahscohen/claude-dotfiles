...
function testPresetWorkflows() {
  ...
  const workflows = PRESET_COMPOSITE_FLOWS;
...
function testRegisterAndRetrieve() {
  const engine = new FlowCompositionEngine();
  const workflow = PRESET_COMPOSITE_FLOWS[0];
...
function testListCompositeFlows() {
  const engine = new FlowCompositionEngine();
  PRESET_COMPOSITE_FLOWS.forEach((wf) => engine.registerCompositeFlow(wf));
  const list = engine.listCompositeFlows();

  results.push({
    test: 'Can list all registered composite flows',
    passed: list.length === PRESET_COMPOSITE_FLOWS.length,
...
function testResearchToImplementationSteps() {
  const workflow = PRESET_COMPOSITE_FLOWS.find((w) => w.id === 'composite_research_to_impl');
  const requiredFlows = [
    'flowA_brand_verify',
  ...
  const hasAllFlows = requiredFlows.every((flow) =>
    workflow?.steps.some((step) => step.flowId === flow)
  );
...
function testQAWorkflowErrorHandling() {
  const workflow = PRESET_COMPOSITE_FLOWS.find((w) => w.id === 'composite_qa_workflow');
...
function testOptimizationWorkflow() {
  const workflow = PRESET_COMPOSITE_FLOWS.find((w) => w.id === 'composite_optimization');
...
function testResultAggregation() {
  ...
  const aggregated = FlowCompositionEngine.aggregateResults(mockResults as any);

  results.push({
    test: 'Result aggregation deduplicates guidance and artifacts',
    passed:
      aggregated.guidance.length === 3 && // Guidance 1, 2, 3 (deduplicated)
...
function testContextPropagation() {
  ...
  const propagated = FlowCompositionEngine.propagateContext(
...
function testUniqueWorkflowIds() {
  const ids = PRESET_COMPOSITE_FLOWS.map((w) => w.id);
