import { PolishStandardValidator, PolishCheckContext, PolishValidationReport } from '../polish-standard-validator';
...
function testPolishStandardValidator() {
  ...
  const report = PolishStandardValidator.validateAll(context);
...
function testExtendedDomainValidator() {
  ...
  const report = ExtendedDomainValidator.validateAll(context);
...
function testCombinedValidation() {
  ...
  const polishReport = PolishStandardValidator.validateAll(context);
  const extendedReport = ExtendedDomainValidator.validateAll(context);
...
async function testFlowJIntegration() {
  ...
  const handler = new FlowJTacticalPolishHandler();
...
async function runValidatorIntegrationTests() {
  ...
  try {
    if (testDomainCheckContextBuilder()) passed++; else failed++;
    if (testPolishStandardValidator()) passed++; else failed++;
    if (testExtendedDomainValidator()) passed++; else failed++;
    if (testCombinedValidation()) passed++; else failed++;
    if (await testFlowJIntegration()) passed++; else failed++;
  } catch (err) {
