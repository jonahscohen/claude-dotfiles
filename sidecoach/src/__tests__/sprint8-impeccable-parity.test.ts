/**
 * Sprint 8 T6: Parameterized parity test across all 22 impeccable verbs.
 *
 * For every entry in IMPECCABLE_VERB_REGISTRY:
...
`,
  ...
  const designSource =
    '/Users/spare3/Documents/Github/claude-dotfiles/reference/DESIGN.md';
...
function flattenOutput(result: any): string {
  if (!result) return '';
  const parts: string[] = [];
  if (result.message) parts.push(String(result.message));
  if (Array.isArray(result.guidance)) {
    for (const g of result.guidance) parts.push(String(g));
  }
  if (Array.isArray(result.flowResults)) {
    for (const fr of result.flowResults) {
      if (!fr) continue;
      if (fr.message) parts.push(String(fr.message));
      if (Array.isArray(fr.guidance)) {
        for (const g of fr.guidance) parts.push(String(g));
      }
      if (Array.isArray(fr.nextSteps)) {
        for (const ns of fr.nextSteps) parts.push(String(ns));
      }
...
async function run() {
  ...
  const perVerbFailCount: Record<string, number> = {};
  const perVerbPassCount: Record<string, number> = {};
  ...
  for (const [verb, entry] of Object.entries(IMPECCABLE_VERB_REGISTRY)) {
    ...
    try {
      ...
      threw = true;
      checks.push([`${verb}: process() threw - ${(e as Error).message}`, false]);
    ...
    if (!threw) {
      ...
      if (returned) perVerbPassCount[verb]++;
      else perVerbFailCount[verb]++;
  ...
  console.log(`\n--- Per-verb breakdown ---`);
  const verbs = Object.keys(IMPECCABLE_VERB_REGISTRY);
  ...
  console.log(
    allPass ? 'sprint8-impeccable-parity PASS' : 'sprint8-impeccable-parity FAIL',
  );
