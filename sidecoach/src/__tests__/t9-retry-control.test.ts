...
async function run() {
  ...
  const sig1 = computeErrorSignature({ validator: 'polish-standard', failedRules: ['r1', 'r2', 'r3'], filePath: '/p' });
  const sig2 = computeErrorSignature({ validator: 'polish-standard', failedRules: ['r3', 'r1', 'r2'], filePath: '/p' });
  expect('signature: deterministic', sig1 === sig2, `${sig1} vs ${sig2}`);
  expect('signature: 12 chars', sig1.length === 12, sig1);
  const sig3 = computeErrorSignature({ validator: 'polish-standard', failedRules: ['r1', 'r2'], filePath: '/p' });
  expect('signature: different rules differ', sig1 !== sig3, `${sig1} vs ${sig3}`);
  const sig4 = computeErrorSignature({ validator: 'multi-lens-audit', failedRules: ['r1', 'r2', 'r3'], filePath: '/p' });
  expect('signature: different validator differs', sig1 !== sig4, `${sig1} vs ${sig4}`);
  const sig5 = computeErrorSignature({ validator: 'polish-standard', failedRules: ['r1', 'r2', 'r3'], filePath: '/other' });
  expect('signature: different file differs', sig1 !== sig5, `${sig1} vs ${sig5}`);

  // === Section 2: readRetryConfig defaults + overrides ===
  const defaultCfg = readRetryConfig({ utterance: '' } as FlowExecutionContext);
  ...
  const overrideCfg = readRetryConfig({ utterance: '', metadata: { retryConfig: { maxCycles: 7, identicalErrorThreshold: 4 } } });
  ...
  const partialCfg = readRetryConfig({ utterance: '', metadata: { retryConfig: { maxCycles: 10 } } });
  expect('config: partial override keeps default for missing', partialCfg.maxCycles === 10 && partialCfg.identicalErrorThreshold === 3);
  const badCfg = readRetryConfig({ utterance: '', metadata: { retryConfig: { maxCycles: -1, identicalErrorThreshold: 0 } } });
  ...
  const freshState = readRetryState({ utterance: '' } as FlowExecutionContext);
  ...
  // with max_cycles.
  let state: RetryState = { cycleCount: 0, errorSignatures: [] };
  const config = DEFAULT_RETRY_CONFIG;
  for (let i = 1; i <= 5; i++) {
    const decision = evaluateHaltConditions(state, config);
    expect(`scenario-1: iteration ${i} should not halt`, !decision.halt, JSON.stringify(decision));
    state = recordIteration(state, `unique-sig-${i}`);
  }
  const capDecision = evaluateHaltConditions(state, config);
  ...
  expect(
    'scenario-1: halt message includes counts',
    typeof capDecision.message === 'string' && capDecision.message.includes('cycleCount=5') && capDecision.message.includes('maxCycles=5'),
    capDecision.message,
  );
  ...
  state = recordIteration(state, sameSig);
  expect('scenario-2: after 1 identical does not halt', !evaluateHaltConditions(state, config).halt);
  // iter 2: 2 entries
  state = recordIteration(state, sameSig);
  expect('scenario-2: after 2 identical does not halt', !evaluateHaltConditions(state, config).halt);
  // iter 3: 3 entries - now should trigger identical_error_loop
  state = recordIteration(state, sameSig);
  const idDecision = evaluateHaltConditions(state, config);
  ...
  expect(
    'scenario-2: message includes signature',
    typeof idDecision.message === 'string' && idDecision.message.includes(sameSig) && idDecision.message.includes('3 identical'),
    idDecision.message,
  );
  ...
  for (let i = 0; i < seq1.length; i++) {
    state = recordIteration(state, seq1[i]);
    const decision = evaluateHaltConditions(state, highMaxConfig);
    expect(
      `scenario-3: A,B,A,B,A,B does not halt after entry ${i + 1} (${seq1[i]})`,
      !decision.halt,
  ...
  for (let i = 0; i < seq2.length; i++) {
    state = recordIteration(state, seq2[i]);
    const decision = evaluateHaltConditions(state, highMaxConfig);
    expect(
      `scenario-4: A,A,B,A,A does not halt after entry ${i + 1} (${seq2[i]})`,
      !decision.halt,
      JSON.stringify(decision),
    );
  }
  // Sanity: but if we add one more A, last 3 become A,A,A => halt
  state = recordIteration(state, 'A');
  const finalDecision = evaluateHaltConditions(state, highMaxConfig);
  expect(
    'scenario-4: sanity - one more A triggers halt',
    finalDecision.halt && finalDecision.reason === 'identical_error_loop',
    JSON.stringify(finalDecision),
  );

  // === Section 8: Polish handler integration - halt on max_cycles ===
  startCapture();
  const polishHandler = new FlowJTacticalPolishHandler();
  const polishHaltCtx: FlowExecutionContext = {
    utterance: 'polish',
  ...
  expect(
    'polish: halt status=error',
    polishResult.status === 'error',
    `status=${polishResult.status} message=${polishResult.message}`,
  );
  expect(
    'polish: halt message mentions max cycles',
    typeof polishResult.message === 'string' && polishResult.message.includes('max cycles'),
    polishResult.message,
  );
  expect(
    'polish: halt log emitted',
    polishLogs.some((l) => l.includes('halted: max cycles reached') && l.includes('cycleCount=5')),
    polishLogs.join('\n'),
  );
  expect(
    'polish: halt result carries retryHalt metadata',
    !!polishResult.executionMetadata?.enhancedContext?.retryHalt &&
      polishResult.executionMetadata.enhancedContext.retryHalt.reason === 'max_cycles',
    JSON.stringify(polishResult.executionMetadata?.enhancedContext),
  );

  // === Section 9: Audit handler integration - halt on identical_error_loop ===
  startCapture();
  const auditHandler = new FlowKMultiLensAuditHandler();
  const auditHaltCtx: FlowExecutionContext = {
    utterance: 'audit',
    projectPath: '/tmp/nonexistent-t9-test',
    metadata: {
      retryState: {
  ...
  expect(
    'audit: halt status=error',
    auditResult.status === 'error',
    `status=${auditResult.status} message=${auditResult.message}`,
  );
  expect(
    'audit: halt message mentions identical',
    typeof auditResult.message === 'string' && auditResult.message.includes('identical attempts'),
    auditResult.message,
  );
  expect(
    'audit: halt log emitted',
    auditLogs.some((l) => l.includes('halted after 3 identical attempts') && l.includes('validator=multi-lens-audit')),
    auditLogs.join('\n'),
  );

  // === Section 10: Critique handler integration - halt on max_cycles ===
  startCapture();
  const critiqueHandler = new FlowLDesignCritiqueHandler();
  const critiqueHaltCtx: FlowExecutionContext = {
    utterance: 'critique',
    projectPath: '/tmp/nonexistent-t9-test',
    metadata: {
  ...
  expect(
    'critique: halt status=error',
    critiqueResult.status === 'error',
    `status=${critiqueResult.status} message=${critiqueResult.message}`,
  );
  expect(
    'critique: halt log emitted',
    critiqueLogs.some((l) => l.includes('halted: max cycles reached')),
    critiqueLogs.join('\n'),
  );

  // === Section 11: Critique handler integration - fresh state, success
  // path attaches retry state to result. ===
  const critiqueOkCtx: FlowExecutionContext = {
    utterance: 'critique',
    projectPath: '/tmp/nonexistent-t9-test',
  };
  ...
  expect(
    'critique: success attaches retryState (cycleCount=1)',
    !!nextStateOnResult && nextStateOnResult.cycleCount === 1 && nextStateOnResult.errorSignatures.length === 1,
    JSON.stringify(nextStateOnResult),
  );
  ...
  const polishOkCtx: FlowExecutionContext = {
    utterance: 'polish',
    projectPath: '/tmp',
    metadata: {},
  };
  ...
  expect(
    'polish: success path returns status=success',
    polishOk.status === 'success',
    `${polishOk.status} ${polishOk.message}`,
  );
  const polishNextState = polishOk.executionMetadata?.enhancedContext?.retryState;
  expect(
    'polish: success attaches retryState (cycleCount=1)',
    !!polishNextState && polishNextState.cycleCount === 1 && polishNextState.errorSignatures.length === 1,
    JSON.stringify(polishNextState),
  );
