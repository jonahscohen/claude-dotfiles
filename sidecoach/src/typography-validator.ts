...
import { loadLineHeightTiers, LineHeightTier } from './reference-loader';
...
interface CssRuleLite {
  ...
  // Alternative shape: array of {property, value}
  properties?: { property: string; value: string }[];
}

const ENUMERATED_RATIOS = [1.125, 1.2, 1.25, 1.333, 1.414, 1.5, 1.618];

function parsePxOrRem(input: string): number | null {
...
function parseUnitless(input: string): number | null {
  if (!input) return null;
  const trimmed = input.trim();
  const num = parseFloat(trimmed);
  if (Number.isFinite(num) && /^-?\d+(?:\.\d+)?\s*$/.test(trimmed)) return num;
  return null;
}

function extractDeclarations(rule: CssRuleLite): Record<string, string> {
  if (rule.declarations) return rule.declarations;
  if (rule.properties) {
    const out: Record<string, string> = {};
    for (const { property, value } of rule.properties) out[property] = value;
    return out;
  }
  return {};
}
 ...
 */
function tierForSize(sizePx: number, tiers: LineHeightTier[]): LineHeightTier | null {
  ...
  for (const tier of tiers) {
    ...
    if (rangeMatch) {
      ...
      if (sizePx >= min && sizePx <= max) return tier;
    }
  }
  return null;
}
...
function isUiSelector(selector: string): boolean {
  return /\b(?:button|input|label|nav|menu|tab|chip|badge|tag)\b/i.test(selector);
}

function isCaptionSelector(selector: string): boolean {
  return /\b(?:caption|small|figcaption|meta|footnote|legend|sub|sup)\b/i.test(selector);
}

function checkLineHeightTier(cssRules: CssRuleLite[], tiers: LineHeightTier[]): TypographyFinding[] {
  ...
  for (const rule of cssRules) {
    ...
    if (lhValue === null) {
      ...
      if (lhPx !== null) lhValue = lhPx / sizePx;
    }
    if (lhValue === null) continue;
...
function checkHeadingSizeByRole(cssRules: CssRuleLite[]): TypographyFinding[] {
  const findings: TypographyFinding[] = [];
...
function checkModularRatio(cssRules: CssRuleLite[], designTokens: any): TypographyFinding[] {
  const findings: TypographyFinding[] = [];
  // Find declared ratio (the project should expose `--type-ratio` or
  // `typography.scale.ratio` in tokens)
  const declaredRatio: number | null =
    designTokens?.typography?.scale?.ratio ??
  ...
  if (declaredRatio !== null) {
    ...
    if (base !== null) {
      ...
      for (const rule of cssRules) {
        ...
        const sizePx = parsePxOrRem(sizeRaw);
        if (sizePx === null) continue;
        if (seenSizes.has(Math.round(sizePx * 100))) continue;
...
export const TypographyValidator = {
  validate(ctx: { cssRules?: CssRuleLite[]; designTokens?: any }): TypographyReport {
    const cssRules = (ctx.cssRules || []) as CssRuleLite[];
    const tiers = loadLineHeightTiers();
    const findings: TypographyFinding[] = [];
 ...
 */
export function typographyFindingsToGuidance(report: TypographyReport): string[] {
  const lines: string[] = [report.summary];
  if (report.findings.length === 0) return lines;
  lines.push('');
