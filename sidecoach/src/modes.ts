...
export const MODE_LIST: Mode[] = [FORGE, KILN, BLOOM, CANVAS, TRIM];
...
export function getModeChain(name: string): FlowId[] | undefined {
  return getMode(name)?.chain;
}

/** Returns the verb chain for a mode name, or undefined if unknown. */
export function getModeVerbChain(name: string): string[] | undefined {
