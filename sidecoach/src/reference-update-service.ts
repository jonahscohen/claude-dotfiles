...
export class ReferenceUpdateService {
  ...
  private userCapturesDir: string;
  private versionCacheFile: string;

  constructor(bundlesDir: string = './bundles', userCapturesDir: string = './user-captured') {
   ...
   */
  private async checkBundleUpdate(bundle: ReferenceBundle): Promise<UpdateCheckResult> {
   ...
   */
  async updateAllBundles(): Promise<UpdateResult[]> {
   ...
   */
  private async updateBundle(bundle: ReferenceBundle): Promise<UpdateResult> {
   ...
   */
  private loadBundle(bundle: ReferenceBundle): any {
   ...
   * Save bundle to file
   */
  private saveBundle(bundle: ReferenceBundle, data: any): void {
    const filePath = path.join(this.bundlesDir, `${bundle.name}.json`);
   ...
   * Validate bundle structure
   */
  private validateBundle(data: any, bundleName: string): boolean {
   ...
   */
  private async updateDESIGNMD(results: UpdateResult[]): Promise<void> {
...
`;
   ...
   */
  private getReferenceBundles(): ReferenceBundle[] {
    return [
      {
 ...
 */
export async function updateReferencesCommand(): Promise<void> {
  ...
  if (updateAvailable) {
    ...
    for (const result of results) {
      if (result.status === 'success') {
        console.log(
          `  [SUCCESS] ${result.bundleName}: ${result.previousVersion} -> ${result.newVersion} (${result.mergedUserCaptures} user captures)`
        );
