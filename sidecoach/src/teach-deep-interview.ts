...
export type DeepFieldId =
  | 'register'
  | 'users'
  | 'brandPersonality'
  | 'antiReferences'
  | 'strategicPrinciples'
  // Deep-only fields (4 extras):
  | 'problem'
  | 'successMetrics'
  | 'businessModel'
  | 'technicalConstraints'
  // Brand-only deep-mode field:
  | 'brandVoice';
 ...
 */
export const CORE_FIELDS: DeepFieldId[] = [
  'register',
  'users',
  'brandPersonality',
  'antiReferences',
 ...
 * ambiguity OMC-style.
 */
export const EXTENDED_FIELDS: DeepFieldId[] = [
  'problem',
  'successMetrics',
  'businessModel',
  'technicalConstraints',
...
export const DEEP_FIELDS: DeepFieldId[] = [...CORE_FIELDS, ...EXTENDED_FIELDS];

/**
 * Maps each field to its OMC-style ambiguity dimension. Used by ambiguityScore
 * to weight missing/low-confidence fields.
 */
export const FIELD_DIMENSION: Record<DeepFieldId, DeepDimension> = {
  register: 'goal',
  users: 'goal',
  problem: 'goal',
 ...
 */
export const VAGUE_PATTERNS: Record<DeepFieldId, RegExp[]> = {
 ...
 * inputs anyway).
 */
export function isVagueAnswer(field: DeepFieldId, answer: string | undefined): boolean {
  if (!answer) return true;
  const trimmed = String(answer).trim();
  if (trimmed.length === 0) return true;
  const patterns = VAGUE_PATTERNS[field] || [];
...
export function dimensionFor(field: DeepFieldId): DeepDimension {
...
export function ambiguityScore(
  confidences: Partial<Record<DeepFieldId, 'high' | 'low' | 'absent'>>,
  fields: DeepFieldId[] = DEEP_FIELDS,
): { ambiguity: number; perDimension: Record<DeepDimension, number>; weakest: DeepDimension | null } {
  const dimensionFields: Record<DeepDimension, DeepFieldId[]> = {
    goal: [],
    constraints: [],
    criteria: [],
    context: [],
  ...
  for (const f of fields) {
    dimensionFields[FIELD_DIMENSION[f]].push(f);
  }
  const dimensionClarity: Record<DeepDimension, number> = {
    goal: 0,
    constraints: 0,
    criteria: 0,
    context: 0,
  };
  for (const dim of Object.keys(dimensionFields) as DeepDimension[]) {
  ...
  const weights: Record<DeepDimension, number> = {
    goal: 0.35,
    constraints: 0.25,
    criteria: 0.25,
    context: 0.15,
  ...
  const clarity =
    dimensionClarity.goal * weights.goal +
  ...
  const order: DeepDimension[] = ['goal', 'constraints', 'criteria', 'context'];
  let weakest: DeepDimension | null = null;
  let weakestScore = 2;
  ...
  if (weakestScore >= 1) weakest = null;
 ...
 */
export function generateFollowUpQuestion(field: DeepFieldId, vagueAnswer: string): string {
 ...
 */
function attributionPattern(): RegExp {
...
export function validateProductMd(
  content: string,
  options: { register?: 'brand' | 'product'; deep?: boolean } = {},
): ProductMdValidation {
...
function extractSectionBody(content: string, section: string): string | null {
 ...
 */
export function parseDeepFlag(brief: string): { brief: string; depth: 'quick' | 'standard' | 'deep' } {
