...
export interface FlowPrerequisite {
  flowId: FlowId;
  required: boolean; // true = must run, false = should run
...
export interface FlowDependency {
  ...
  contextRequirements?: string[]; // e.g., ['brandPersonality', 'componentTree']
  minSuccessfulPrerequisites?: number; // if not all required
}

const FLOW_DEPENDENCIES: Map<FlowId, FlowDependency> = new Map([
  ...
  static canExecute(
    ...
    const completedFlows = new Set(
      flowHistory
        ...
        .map((entry) => entry.flowId as FlowId)
    );
    ...
    if (deps.minSuccessfulPrerequisites) {
      const successfulOptional = optionalPrereqs.filter((p) =>
        completedFlows.has(p.flowId)
      ).length;
