...
import { CRITIQUE_RULES } from './design-laws';
  ...
  async execute(context: FlowExecutionContext): Promise<FlowExecutionResult> {
    ...
    try {
      ...
      if (fs.existsSync(srcDir)) {
        ...
        const relevantFiles = files.filter((f: string | Buffer) => {
        ...
        if (relevantFiles.length > 0) {
          ...
          if (Object.keys(codeBlocks).length > 0) {
            ...
            const antiPatternFindings: string[] = [];
            ...
            for (const [file, result] of Object.entries(batchResults)) {
              if (result.violations.length > 0) {
                ...
                for (const violation of result.violations.slice(0, 3)) {
                  ...
                  antiPatternFindings.push(
                    `  [${violation.severity}] ${violation.patternName}: ${violation.fix}`
                  );
                }
...
export class FlowLDesignCritiqueHandler extends BaseFlowHandler {
  ...
  async execute(context: FlowExecutionContext): Promise<FlowExecutionResult> {
    ...
    const reflexDetector = new CategoryReflexDetector();
    const artifacts: FlowArtifact[] = [];
    ...
    if (enhancedContext?.flowMetadata) {
      ...
      enhancedContext.flowMetadata.customData = {
        'critique-rules': CRITIQUE_RULES.length,
    ...
    const critiqueBases: { rule: string; weight: number }[] = CRITIQUE_RULES.map((r) => ({
      rule: r.name,
      weight: r.weight,
    }));
    ...
    const critiqueContent = `12-Rule Critique Framework:\n${CRITIQUE_RULES.map(
      (r) => `${r.name} (weight: ${r.weight})\n  ${r.description}`
    ).join('\n\n')}`;

    artifacts.push(
      this.createArtifact('reference', 'critique-framework', critiqueContent, 'Weighted 12-rule critique framework')
    );
...
export class FlowMResponsiveValidationHandler extends BaseFlowHandler {
  ...
  async execute(context: FlowExecutionContext): Promise<FlowExecutionResult> {
    ...
    try {
      ...
      if (fs.existsSync(srcDir)) {
        const files = fs.readdirSync(srcDir, { recursive: true });
        const cssFiles = files.filter((f: string | Buffer) => {
          const fname = typeof f === 'string' ? f : f.toString();
        ...
        if (cssFiles.length > 0) {
          ...
          if (cssContent.length > 0) {
            ...
            const responsiveViolations = result.violations.filter(
              (v) =>
                v.patternName.includes('mobile') ||
...
export class FlowNRapidIterationHandler extends BaseFlowHandler {
  ...
  async execute(context: FlowExecutionContext): Promise<FlowExecutionResult> {
    ...
    try {
      ...
      if (fs.existsSync(srcDir)) {
        const files = fs.readdirSync(srcDir, { recursive: true });
        const codeFiles = files.filter((f: string | Buffer) => {
          const fname = typeof f === 'string' ? f : f.toString();
        ...
        if (codeFiles.length > 0) {
          ...
          if (Object.keys(codeBlocks).length > 0) {
            ...
            const iterationViolations: string[] = [];
            for (const [file, result] of Object.entries(batchResults)) {
              const violations = result.violations.filter(
                (v) =>
                  v.patternName.includes('Inconsistent') ||
