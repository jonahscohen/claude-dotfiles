...
export class FlowMResponsiveValidationHandler extends BaseFlowHandler {
  ...
  async execute(context: FlowExecutionContext): Promise<FlowExecutionResult> {
    ...
    try {
      ...
      const responsiveDoc = loadCanonical('responsive-foundation');
      const breakpoints = loadBreakpointTable();
      ...
      const artifacts = [
        this.createArtifact(
          'reference',
          'Bencium Breakpoint Table',
          breakpoints.map((b) => `${b.name} (${b.range}): primary=${b.primaryPattern} | nav=${b.navPattern} | table=${b.tablePattern}`).join('\n'),
      ...
      if (responsiveDoc) {
        artifacts.push(
          this.createArtifact(
            'reference',
            'responsive-foundation.md (canonical)',
            responsiveDoc.length > 4000 ? responsiveDoc.slice(0, 4000) + '\n\n[truncated - see sidecoach/reference/responsive-foundation.md for full content]' : responsiveDoc,
            'The full canonical responsive reference. Includes mobile nav patterns (hamburger/bottom-tab/drawer/segmented/tab-bar/command-palette/FAB), iOS Safari gotchas, fluid type, container queries vs media queries, and the responsive degrade plan template.',
          ),
        );
      }
