...
import { createReferenceDataService, ReferenceDataService } from './reference-data';
...
export class FlowBComponentResearchHandler extends BaseFlowHandler {
  ...
  constructor() {
    ...
    this.refData = createReferenceDataService();
  }
...
export class FlowFDesignTokensHandler extends BaseFlowHandler {
  ...
  async execute(context: FlowExecutionContext): Promise<FlowExecutionResult> {
    ...
    const sampleTokens = hasSections
      ? sections.slice(0, 3).map((s) => `${s} (${Object.keys((tokens as any)[s]).length} tokens)`)
      : [];
...
export class FlowHMotionIntegrationHandler extends BaseFlowHandler {
  ...
  async execute(context: FlowExecutionContext): Promise<FlowExecutionResult> {
    ...
    const patternName = (context.metadata?.motionPattern as string) || 'fade_in';
