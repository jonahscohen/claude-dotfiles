...
export class FlowQMigrationHandler extends BaseFlowHandler {
  constructor() {
    super('flowQ_migration_special' as any);
  }
      ...
      return {
        ...
        artifacts: [
          this.createArtifact(
            'reference',
            'Migration Guide',
            'Old API → New API mappings with breaking change notes',
            'Design system API migration documentation'
          ),
          this.createArtifact(
            'reference',
            'Dependency Graph',
            'Component dependency map for migration order planning',
...
export function createFlowQHandler(): FlowQMigrationHandler {
